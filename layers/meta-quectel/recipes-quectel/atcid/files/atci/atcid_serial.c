...
void writeDataToserialByResponseType(ATRESPONSE_t response_type) {
    ...
    switch(response_type) {
        ...
        case AT_NOT_IMPL:
            ...
            break;
        case AT_QUEC_QUITE:
            //Quectel:return from modem ,No more print from AP
            return;
            break;
	case AT_ERROR4 :
            memcpy(response+i, AT_ERROR4_STRING, strlen(AT_ERROR4_STRING));
            i+=strlen(AT_ERROR4_STRING);
	    break;
        default:
...
#if defined(ANDROID) || defined(TELEMATICS)
...
void connectTarget(ATCI_DataType dataType) {
    LOGATCI(LOG_DEBUG,"Enter %d",dataType);
...
#else
        //TODO:
        s_fdRild_command = connectToMPCI();
        if (s_fdRild_command < 0) {
            if (errno == ENOENT) { // No such file or directory
                ALOGE("ENOENT[%d] : No such file or directory",ENOENT);
                isRilAtciSocketExist = false;
...
///* for read rild data */
        ...
        LOGATCI(LOG_DEBUG, "Maxfd %d",maxFd);
// ///* for read rild data */
...
// ////
        ...
        for(i = 0; i < MAX_DEVICE_NUM; i++) {
            if (serial->fd[i] != INVALIDE_SOCKET_FD && FD_ISSET(serial->fd[i], &rfds)) {
                ...
                if(ADB_SKT_SERVER_NUM == currDevice) {
                    LOGATCI(LOG_DEBUG, "currDevice = ADB_SKT_SERVER_NUM");
                    serial->fd[ADB_SKT_CLIENT_NUM] = adb_socket_accept(fd);
                    atci_conn_mode = CONN_ADB;
                    p_buffer = tmpBuffer;
                } else if(ADB_SKT_CLIENT_NUM == currDevice) {
                    LOGATCI(LOG_DEBUG, "currDevice = ADB_SKT_CLIENT_NUM");
                    atci_conn_mode = CONN_ADB;
                    memset(tmpBuffer, 0, sizeof(tmpBuffer));
                    p_buffer = tmpBuffer;
                } else if (HIDL_CONN_NUM == currDevice) {
                    LOGATCI(LOG_DEBUG, "currDevice = CONN_HIDL");
                    atci_conn_mode = CONN_HIDL;
                    memset(tmpBuffer, 0, sizeof(tmpBuffer));
                    p_buffer = tmpBuffer;
                } else {
                    LOGATCI(LOG_DEBUG, "currDevice = CONN_VCOM");
                    atci_conn_mode = CONN_VCOM;
                    memset(buffer, 0, sizeof(buffer));
                    p_buffer = buffer;
                }
                LOGATCI(LOG_DEBUG,"buffer:%s,fd:%d",p_buffer,serial->fd[i]);
                break;
            }
        }

        // if(fd == INVALIDE_SOCKET_FD){
        //     LOGATCI(LOG_DEBUG, "ERROR:No data from uart devices");
        //     continue;
        // }
        ...
        //     }
        //     else if(readCount <= 2) {
        //         LOGATCI(LOG_DEBUG, "readCount <1 continue");
        //         break;
        //     }
        // } while(1);
        readCount = read(fd, p_buffer, MAX_DATA_SIZE);
        if (readCount <= 2) {
            LOGATCI(LOG_DEBUG, "ERROR:No data from USB devices");
            reload_time++;
            if(reload_time<20){
                LOGATCI(LOG_DEBUG, "Try to get data in next loop");
                continue;
            }else{
                LOGATCI(LOG_DEBUG, "Reload uart fd :%d",fd);
                reload_time = 0;
        ...
        if (memcmp(p_buffer, "FCT", 3) == 0) { writeDataToSerial("OK\r\n", 4); }
...
void *readerLoop(void *arg) {
    ...
    for (;;) {
        ...
        else if(dataType == QL_AT_TYPE){
            LOGATCI(LOG_INFO, "Quectel AT COMMAND");
        }
    }
...
int open_serial_device(Serial* serial, char* devicename) {
    ...
    if(strcmp(devicename, TTY_GS0) == 0) {
        ...
        LOGATCI(LOG_ERR,"is echo TTYGS0");
    }
    ...
    //deviceOptions.c_iflag &= ~(INLCR | IGNCR | ICRNL);            //igni remove for HT tool
    deviceOptions.c_iflag |= ICRNL;  //replace'\r'->'\n'
...
int readSys_int(char const * path) {
    ...
    if (fd >= 0) {
        char buffer[128] = {0};
        int amt = read(fd, buffer, 128);
        close(fd);
        char *mode = strrchr(buffer,':');
        char *isMeda = strstr(mode, "META BOOT");

        LOGATCI(LOG_DEBUG, "boot mode     = <%s>, isMeda = [%s]", mode, isMeda);

        if (isMeda != NULL) {
            LOGATCI(LOG_DEBUG, "boot mode = 1");
            return 1;
        }
        LOGATCI(LOG_DEBUG, "boot mode = 0");
        return 0;
...
int get_control_socket(char* name) {

    struct sockaddr_un server;

    int fd = socket(AF_UNIX, SOCK_STREAM, 0);
    if (fd < 0) {
        LOGATCI(LOG_ERR, "Can't open stream socket (%s)", name);
        return 0;
    }
    ...
    if (bind(fd, (struct sockaddr *) &server, sizeof(struct sockaddr_un)) < 0) {
        LOGATCI(LOG_ERR, "bind failed name=[%s] reason=[%s]\n", name, strerror(errno));
        close(fd);
        return 0;
    }
    return fd;
