			       ...
			       struct modem_ctrl_device *mcd_dev)
{
	...
	if (cp_gpio == mcd_dev->init->gpio_cpwatchdog) {
		...
		ret = devm_request_threaded_irq(
			mcd_dev->dev, mcd_dev->init->irq_cpwatchdog, NULL,
			cpwatchdogtriger_handler,
			IRQF_ONESHOT | IRQF_TRIGGER_FALLING, "cpwatchdog_irq",
			mcd_dev);
		if (ret < 0) {
			dev_err(mcd_dev->dev,
				"can not request irq for cp watchdog\n");
			return ret;
		}
		...
		ret = devm_request_threaded_irq(
			mcd_dev->dev, mcd_dev->init->irq_cpassert, NULL,
			cpasserttriger_handler,
			IRQF_ONESHOT | IRQF_TRIGGER_FALLING, "cpassert_irq",
			mcd_dev);
		if (ret < 0) {
			dev_err(mcd_dev->dev,
				"can not request irq for cp assert\n");
			return ret;
		}
		...
		ret = devm_request_threaded_irq(
			mcd_dev->dev, mcd_dev->init->irq_cppanic, NULL,
			cppanictriger_handler,
			IRQF_ONESHOT | IRQF_TRIGGER_FALLING, "cppanic_irq",
			mcd_dev);
		if (ret < 0) {
			dev_err(mcd_dev->dev,
...
void modem_ctrl_send_abnormal_to_ap(int status)
{
	...
	dev_info(mcd_dev->dev, "operation unnormal status %d send to ap\n",
		 status);
	if (!IS_ERR(gpiodesc))
...
static void modem_ctrl_send_cmd_to_cp(int status)
{
	...
	mcd_dev->init->modem_status = status;
	dev_info(mcd_dev->dev, "operation  cmd %d ms send to cp\n", status);
	if (!IS_ERR(gpiodesc)) {
		gpiod_set_value_cansleep(gpiodesc, 0);
		msleep(20);
...
void modem_ctrl_poweron_modem(int on)
{
	...
	case MDM_CTRL_PCIE_RECOVERY:
		...
		break;
	case MDM_POWER_OFF:
		atomic_notifier_call_chain(&modem_ctrl_chain, MDM_POWER_OFF,
					   NULL);
...
static int modem_ctrl_open(struct inode *inode, struct file *filp)
{
	...
	modem_ctrl =
		container_of(inode->i_cdev, struct modem_ctrl_device, cdev);
...
static ssize_t modem_ctrl_read(struct file *filp, char __user *buf,
...
static ssize_t modem_ctrl_write(struct file *filp, const char __user *buf,
				size_t count, loff_t *ppos)
{
	char sbuf[100];
	int ret;
				     ...
				     struct device *dev)
{
	...
	pdata->gpio_cpwatchdog =
		devm_gpiod_get(dev, "cpwatchdog", GPIOD_OUT_HIGH);
			       ...
			       struct device *dev)
{
	...
	pdata->gpio_cppoweroff =
		devm_gpiod_get(dev, "cppoweroff", GPIOD_OUT_HIGH);
	if (IS_ERR(pdata->gpio_cpassert))
		return PTR_ERR(pdata->gpio_cppoweroff);
...
static inline void modem_ctrl_destroy_pdata(struct modem_ctrl_init_data **init)
{
...
static int modem_ctrl_probe(struct platform_device *pdev)
{
	...
	dev_dbg(dev, "after parse device tree, name=%s soctype=%d\n",
		init->name, modem_ctrl_dev->soc_type);
...
error0:
	device_destroy(modem_ctrl_class,
		       MKDEV(modem_ctrl_dev->major, modem_ctrl_dev->minor));
error1:
...
static int modem_ctrl_remove(struct platform_device *pdev)
{
	...
	unregister_chrdev_region(
		MKDEV(modem_ctrl_dev->major, modem_ctrl_dev->minor), 1);
