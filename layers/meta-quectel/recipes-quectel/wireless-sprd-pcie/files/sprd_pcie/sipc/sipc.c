...
enum { NORMAL_MODE = 0, CHARGE_MODE, CALI_MODE };
...
static struct device *pcie_dev = NULL;
#define SHARE_MEM_SIZE 0x300000
...
static int sipc_parse_dt(struct smsg_ipc *ipc, struct device_node *np,
			 struct device *dev)
{
	u32 val[3];
	int ret;
	...
	if (ipc->type == SIPC_BASE_MBOX) {
		...
		if (of_property_read_u32_array(np, "sprd,core_sensor", val,
					       1) == 0) {
	...
	if (ipc->type == SIPC_BASE_PCIE) {
		...
		ret = of_property_read_u32_array(np, "sprd,ep-dev",
						 &ipc->ep_dev, 1);
...
#ifdef CONFIG_PCIE_EPF_SPRD
	if (ipc->type == SIPC_BASE_PCIE) {
		ret = of_property_read_u32_array(np, "sprd,ep-fun",
						 &ipc->ep_fun, 1);
	...
	/* get smem type */
	ret = of_property_read_u32_array(np, "sprd,smem-type", &val[0], 1);
	if (!ret)
	...
	/* get smem info */
	ret = of_property_read_u32_array(np, "sprd,smem-info", val, 3);
	if (ret) {
	...
	ret = of_property_read_u32(np, "sprd,high-offset", val);
	if (!ret) {
...
{
	int ret = 0;
	dma_addr_t smem_hwaddr;
	void *smem_vaddr;

	smem_vaddr = dma_alloc_coherent(pcie_dev, SHARE_MEM_SIZE, &smem_hwaddr,
					GFP_KERNEL);
	if (!smem_vaddr) {
		pr_info("sipc init dma_alloc_coherent 3M fail\n");
		return -1;
	}
...
static int sipc_probe(struct platform_device *pdev)
{
	...
	if (1) {
		np = pdev->dev.of_node;
		ipc = devm_kzalloc(&pdev->dev, sizeof(struct smsg_ipc),
				   GFP_KERNEL);
		if (!ipc)
			return -ENOMEM;
...
static int sipc_remove(struct platform_device *pdev)
{
	...
	dma_free_coherent(pcie_dev, SHARE_MEM_SIZE, (void *)ipc->smem_base,
			  (dma_addr_t)ipc->dst_smem_base);
...
int sipc_init(struct device *dev)
{
	int ret;
