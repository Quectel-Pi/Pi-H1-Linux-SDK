...
{
	struct sipa_core *sipa_ctrl =
		container_of(work, struct sipa_core, flow_ctrl_work);
			      ...
			      enum sipa_cmn_fifo_index id)
{
	...
	cmn_fifo->tx_fifo.virtual_addr =
		dma_alloc_coherent(ipa->pci_dev, size, &dma_addr, GFP_KERNEL);
	...
	pr_info("comfifo%d tx_fifo addr-0x%lx\n", id,
		(long unsigned int)cmn_fifo->tx_fifo.virtual_addr);
	...
	pr_info("comfifo%d rx_fifo addr-0x%lx\n", id,
		(long unsigned int)cmn_fifo->rx_fifo.virtual_addr);
...
static void sipa_free_cmn_fifo(struct sipa_core *ipa,
			       enum sipa_cmn_fifo_index id)
{
	size_t size;
	struct sipa_cmn_fifo_cfg_tag *cmn_fifo;
	...
	size = cmn_fifo->tx_fifo.depth *
	       sizeof(struct sipa_node_description_tag);
	dma_free_coherent(ipa->dev, size, cmn_fifo->tx_fifo.virtual_addr,
			  cmn_fifo->tx_fifo.dma_ptr);
	size = cmn_fifo->rx_fifo.depth *
	       sizeof(struct sipa_node_description_tag);
	dma_free_coherent(ipa->dev, size, cmn_fifo->rx_fifo.virtual_addr,
			  cmn_fifo->rx_fifo.dma_ptr);
}
					...
					struct sipa_core *ipa)
{
	...
	ipa->reg_res =
		platform_get_resource_byname(pdev, IORESOURCE_MEM, "ipa-base");
	if (!ipa->reg_res) {
	...
	if (ret) {
		dev_err(&pdev->dev, "get pcie-dl-tx-fifo-depth ret = %d\n",
			ret);
		return ret;
	}
	...
	if (ret) {
		dev_err(&pdev->dev, "get pcie-dl-rx-fifo-depth ret = %d\n",
			ret);
		return ret;
	}
	...
	ret = of_property_read_u32(pdev->dev.of_node, "pcie-ul-tx-fifo-depth",
				   &cmn_fifo->tx_fifo.depth);
	if (ret) {
		dev_err(&pdev->dev, "get pcie-ul-tx-fifo-depth ret = %d\n",
			ret);
		return ret;
	}
	...
	ret = of_property_read_u32(pdev->dev.of_node, "pcie-ul-rx-fifo-depth",
				   &cmn_fifo->rx_fifo.depth);
	if (ret) {
		dev_err(&pdev->dev, "get pcie-ul-rx-fifo-depth ret = %d\n",
			ret);
		return ret;
	}
