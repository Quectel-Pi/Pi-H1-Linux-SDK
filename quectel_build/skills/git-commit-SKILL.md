...
- **`<软件测试>` 没有约定测试时一律写 `N`**（默认值就是 `N`）
  - 用户没有明确约定测试 → 一律写 `N`，不要自行编造测试方法或测试步骤
  - 不要因为"自己做过验证"就写 `Y`；`Y` 只用于用户明确约定/要求测试的场景
- 只有用户明确约定了测试（说明要测什么、怎么验证）时才写 `Y`，并在下面两行写明测试方法、测试步骤：
...
- 例如：修改 AT 指令且用户要求验证 → `<AP侧AT指令新增或修改>: Y`，写明测试指令和预期结果
- 例如：仅文档修改 → 所有测试项填 `N`
- `<压力测试>` / `<硬件测试>` / `<AP侧AT指令新增或修改>` 同样遵循"没约定就写 `N`"
