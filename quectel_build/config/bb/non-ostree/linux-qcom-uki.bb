...
do_compile() {
    ...
    QCOM_BOOTIMG_ROOTFS="/dev/sda3"
    KERNEL_CMDLINE_EXTRA="console=tty0 pcie_pme=nomsi kernel.sched_pelt_multiplier=4 rcupdate.rcu_expedited=1 rcu_nocbs=0-7 kpti=off kasan=off kasan.stacktrace=off no-steal-acc ${DBG_CMDLINE} swiotlb=128 mitigations=auto net.ifnames=0"

    echo "DISTRO_FEATURES=$DISTRO_FEATURES"
    echo "QCOM_BOOTIMG_ROOTFS=$QCOM_BOOTIMG_ROOTFS"
    echo "SERIAL_CONSOLES=$SERIAL_CONSOLES"
    echo "KERNEL_CMDLINE_EXTRA=$KERNEL_CMDLINE_EXTRA"

    # Kernel cmdline
    cmdline=""
    if [ -n "${QCOM_BOOTIMG_ROOTFS}" ]; then
        cmdline="$cmdline root=/dev/sda3 rw rootwait"
    fi
