...
if __name__ == '__main__':
    ...
    parser.add_option('', '--kimage-voffset', type='int',
                      dest='kimage_voffset',
                      help='Force the kernel image virtual-to-physical offset '
                           '(kimage_voffset = kimage_vaddr - kernel_text_phys). '
                           'For KASLR dumps where auto-detection fails: derive it '
                           'as (linux_banner vaddr) - (linux_banner phys in DDR).')
    ...
    if not options.minidump:
        if not dump.print_command_line():
            print_out_str('!!! Warning: could not print saved command line '
