    ...
    def _parse(self):
        ...
        with open(os.path.join(self.autodumpdir, filename)) as f:
            for line in f.readlines():
                ...
                size = int(words[2])
                fullpath = os.path.join(self.autodumpdir, fname)
                try:
                    filesize = os.path.getsize(fullpath)
                except OSError:
                    print_out_str(
                        '!!! %s listed in dump_info.txt but not present. Skipping...' % fname)
                    continue
                if size != filesize:
                    print_out_str(
...
class RamDump():
    ...
    def __init__(self, options, nm_path, gdb_path, objdump_path,gdb_ndk_path):
        ...
        if self.arm64:
            ...
            if self.kimage_voffset is not None:
                ...
                # --kimage-voffset: 手动指定 KASLR offset(用于自动推算失败的 KASLR dump)
                # 用法: (linux_banner vaddr) - (linux_banner 物理地址)
                if getattr(options, 'kimage_voffset', None) is not None:
                    self.kimage_voffset = options.kimage_voffset
    ...
    def sizeof(self, the_type):
        try:
            sz = self.gdbmi.sizeof(the_type)
        except (gdbmi.GdbMIException, AttributeError):
            sz = None
            if self.hyp and self.gdbmi_hyp is not None:
                try:
                    sz = self.gdbmi_hyp.sizeof(the_type)
                except gdbmi.GdbMIException:
                    pass
        # Workaround: some host gdb versions do not support aarch64 and
        # report 4-byte pointers/longs for arm64 vmlinux (void */long/
        # unsigned long).  Force the correct width based on the target
        # architecture so all parsers computing pointer strides with
        # sizeof() work on such dumps.
        if sz is not None and sz == 4 and self.arm64:
            if the_type in ('void *', 'unsigned long', 'long',
