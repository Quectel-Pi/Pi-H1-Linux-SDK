 ...
 */
 ...
 int dsi_display_dev_probe(struct platform_device *pdev)
 {
	 ...
	 bool firm_req = false, ext_disp_en = false, switch_ws_panel = false;
	 struct dsi_display_boot_param *boot_disp;
 
	 int data;
	 int panel_type = 0;
 
	 const struct WriteCommand ws_reg[] = {
		 {0xc0, 0x01},
		 {0xcc, 0x01},
		 {0xc2, 0x01},
		 {0xac, 0x01},
	 ...
	 const struct WriteCommand ili_reg[] = {
		 {0x02, 0x03},
		 {0x03, 0x9f}
	 };
     ...
     int gpio15 = of_get_named_gpio(pdev->dev.of_node, "gpio15_dsi", 0);
     if (!gpio_is_valid(gpio15)) {
         DSI_ERR("%s: my_gpio15-gpios not specified or invalid (%d)\n", __func__, gpio15);
     } else {
         int ret = devm_gpio_request_one(&pdev->dev, gpio15, GPIOF_OUT_INIT_HIGH, "gpio15_dsi");
         if (ret) {
             DSI_ERR("%s: failed to request gpio15 (%d)\n", __func__, ret);
         } else {
             DSI_INFO("%s: gpio15 (%d) set to output HIGH\n", __func__, gpio15);
         }
     } 
	 ...
	 if (data < 0) {
		 DSI_INFO( "Failed to read REG_ID reg: %d\n", data);
		 switch_ws_panel = false;
	 }
	 else
	 {
		 DSI_INFO("success to read REG_ID reg: 0x%02x\n", data & 0xFF);
		 ...
		 switch (data & 0x0f) {
			 case 0x01: /* 7 inch */
				 ...
				 if(data == 4){
					 ...
					 DSI_ERR( "Using ws panel %02x\n",data);
				 }
				 else{
					 panel_type = LCD_PANEL_RPI_7INCH;
					 DSI_ERR( "Using 7 inch panel%02x\n",data);
				 }
				 break;
			 case 0x04: /* 7 inch - old */
				 DSI_ERR( "Using 7 inch (old) panel\n");
				 panel_type = LCD_PANEL_RPI_7INCH_OLD;
				 break;
			 case 0x08: /* 5 inch - old */
				 DSI_ERR( "Using 5 inch (old) panel\n");
				 panel_type = LCD_PANEL_RPI_7INCH_OLD;
				 break;
			 case 0x09: /* 5 inch */
				 DSI_ERR( "Using 5 inch panel\n");
	 ...
	 if (boot_disp->boot_disp_en) {
		 ...
		 if (switch_ws_panel) {
			 switch (panel_type) {
				 case LCD_PANEL_RPI_7INCH:
					 for (size_t i = 0; i < sizeof(ili_reg) / sizeof(ili_reg[0]); ++i) {
						 if (ws_panel_i2c_write(ili_reg[i].address, ili_reg[i].data)) {
							 DSI_ERR("I2C write failed at (address: 0x%02X, data: 0x%02X)\n", ili_reg[i].address, ili_reg[i].data);
							 break;
						 }
					 }     
					 panel_node = of_parse_phandle(node,"qcom,dsi-select-ili9881-panel", 0);
					 break;
				 case LCD_PANEL_WAVESHARE:
					 for (size_t i = 0; i < sizeof(ws_reg) / sizeof(ws_reg[0]); ++i) {
						 if (ws_panel_i2c_write(ws_reg[i].address, ws_reg[i].data)) {
							 DSI_ERR("I2C write failed at (address: 0x%02X, data: 0x%02X)\n", ws_reg[i].address, ws_reg[i].data);
							 break;
						 }
					 }     
					 panel_node = of_parse_phandle(node,"qcom,dsi-select-ws-panel", 0);
					 break;
				 default:
					 rc = -EPROBE_DEFER;
					 DSI_ERR("Unknow panel go fail%d\n", rc);
					 goto end;
			 }
