...
static int dsi_panel_reset(struct dsi_panel *panel)
{
	...
	if (!(strcmp(panel->name, "waveshare lcd video mode dsi panel without DSC")))
		return 0;
...
#if 0
		DSI_INFO(" enter %s %d i:%d flags:0x%x, channel:%d, tx_len:%zu, rx_len:%zu\n",
			__func__, __LINE__, i,
			cmds->msg.flags,
			cmds->msg.channel,
			cmds->msg.tx_len,
			cmds->msg.rx_len);
#endif	
		const u8 *tx = cmds->msg.tx_buf;
		const u8 *rx = cmds->msg.rx_buf;
	
		if (tx && cmds->msg.tx_len > 0) {
			char tx_buf[512] = {0};
			int pos = 0;
			for (size_t j = 0; j < cmds->msg.tx_len && pos < sizeof(tx_buf) - 3; j++)
				pos += scnprintf(tx_buf + pos, sizeof(tx_buf) - pos, "%02x ", tx[j]);
			//DSI_INFO("TX: %s\n", tx_buf);
		}
	
		if (rx && cmds->msg.rx_len > 0) {
			char rx_buf[512] = {0};
			int pos = 0;
			for (size_t j = 0; j < cmds->msg.rx_len && pos < sizeof(rx_buf) - 3; j++)
				pos += scnprintf(rx_buf + pos, sizeof(rx_buf) - pos, "%02x ", rx[j]);
			//DSI_INFO("RX: %s\n", rx_buf);
		}
	
		len = dsi_host_transfer_sub(panel->host, cmds);
		//DSI_INFO(" dsi_host_transfer_sub len: %zd\n", len);
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	case DSI_BACKLIGHT_EXTERNAL:
		rc = ws_panel_set_backlight(bl_lvl);
		break;
