...
static ssize_t spipe_read(struct file *filp, char __user *buf, size_t count,
			  loff_t *ppos)
{
	struct spipe_sbuf *sbuf = filp->private_data;
	int timeout = -1;
	...
	return sbuf_read(sbuf->dst, sbuf->channel, sbuf->bufid, (void *)buf,
			 count, timeout);
}

static ssize_t spipe_write(struct file *filp, const char __user *buf,
			   size_t count, loff_t *ppos)
{
	...
	return sbuf_write(sbuf->dst, sbuf->channel, sbuf->bufid, (void *)buf,
			  count, timeout);
}
...
static unsigned int spipe_poll(struct file *filp, poll_table *wait)
{
	...
	return sbuf_poll_wait(sbuf->dst, sbuf->channel, sbuf->bufid, filp,
			      wait);
}
...
static int spipe_parse_dt(struct spipe_init_data **init, struct device_node *np,
			  struct device *dev, dev_t *devid)
{
	...
	ret = of_property_read_u32(np, "sprd,size-rxbuf",
...
static int spipe_probe(struct platform_device *pdev)
{
	...
	if (1) {
		...
		printk("spipe: after parse device tree, name=%s, dst=%u, channel=%u, ringnr=%u,  rxbuf_size=0x%x, txbuf_size=0x%x\n",
		       init->name, init->dst, init->channel, init->ringnr,
		       init->rxbuf_size, init->txbuf_size);
		...
		spipe = devm_kzalloc(&pdev->dev, sizeof(struct spipe_device),
				     GFP_KERNEL);
		if (spipe == NULL) {
		...
		if (init->ringnr > 1) {
			...
			device_create(spipe_class, NULL,
				      MKDEV(spipe->major, spipe->minor), NULL,
				      "%s", init->name);
		}
...
static int spipe_remove(struct platform_device *pdev)
{
	...
	if (spipe) {
		...
		cdev_del(&(spipe->cdev));
		unregister_chrdev_region(MKDEV(spipe->major, spipe->minor),
					 spipe->init->ringnr);
