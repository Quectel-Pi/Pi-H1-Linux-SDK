...
static int sipa_arp_reply(struct net_device *net, struct sk_buff *skb)
{
	...
	if (parp->ar_hrd == htons(ARPHRD_ETHER) &&
	    ...
	    parp->ar_op == htons(ARPOP_REQUEST) && parp->ar_hln == 6 &&
	    parp->ar_pln == 4) {
		...
		pr_info("%s sip = %d.%d.%d.%d, tip=%d.%d.%d.%d\n",
			netdev_name(net), sip[0], sip[1], sip[2], sip[3],
			tip[0], tip[1], tip[2], tip[3]);
		reply = arp_create(ARPOP_REPLY, ETH_P_ARP, *((__be32 *)sip),
				   skb->dev, *((__be32 *)tip), sha,
				   net->dev_addr, sha);
		if (reply) {
...
static void sipa_get_modem_mac(struct sk_buff *skb, struct SIPA_ETH *sipa_eth)
{
	...
	if (ehdr->h_proto == htons(ETH_P_IP) && iph->protocol == IPPROTO_UDP &&
	    iph->saddr != 0x00000000 && iph->daddr == 0xFFFFFFFF) {
		if (udph->dest == htons(68)) //DHCP offer/ACK
		{
			...
			pr_info("Modem Mac Address: %02x:%02x:%02x:%02x:%02x:%02x\n",
				pdata->modem_mac[0], pdata->modem_mac[1],
				pdata->modem_mac[2], pdata->modem_mac[3],
				pdata->modem_mac[4], pdata->modem_mac[5]);
		}
	}
}
#endif
...
static int sipa_dummy_rx_poll_handler(struct napi_struct *napi, int budget)
{
	...
	struct SIPA_DUMMY *sipa_dummy =
		container_of(napi, struct SIPA_DUMMY, napi);
...
static int sipa_dummy_probe(struct platform_device *pdev)
{
	...
	netdev = alloc_netdev(sizeof(struct SIPA_DUMMY), "sipa_dummy0",
			      NET_NAME_PREDICTABLE, s_setup);
#else
	netdev =
		alloc_netdev(sizeof(struct SIPA_DUMMY), "sipa_dummy0", s_setup);
#endif
...
static int sipa_dummy_debugfs_mknod(void *data)
{
	...
	debugfs_create_file("stats", 0444, dummy_root, data,
