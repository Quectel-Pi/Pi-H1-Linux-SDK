...
static const unsigned char dhcp_dst_addr[ETH_ALEN] = { 0xff, 0xff, 0xff,
						       0xff, 0xff, 0xff };
...
static inline void sipa_eth_rx_stats_update(struct sipa_eth_dtrans_stats *stats,
					    u32 len)
{
	stats->rx_sum += len;
	stats->rx_cnt++;
...
static inline void sipa_eth_tx_stats_update(struct sipa_eth_dtrans_stats *stats,
					    u32 len)
{
	stats->tx_sum += len;
	stats->tx_cnt++;
	...
	if (dev->type != ARPHRD_ETHER) {
		...
		case 0x40: {
			...
			if (ip4h->protocol == IPPROTO_UDP &&
			    ip4h->saddr == 0x00000000 &&
			    ip4h->daddr == 0xFFFFFFFF) {
				dhcp = 1;
			}
		...
		if (dhcp)
			...
			memcpy(eth_hdr(skb)->h_dest, pdata->modem_mac,
			       ETH_ALEN);
...
static int sipa_eth_change_dev_list_state(struct SIPA_ETH *sipa_eth, int state)
{
	...
	if (strncmp(sipa_eth->netdev->name, SIPA_ETH_VPCIE_PREF,
	...
	pr_info("set %s netid %d %s for dummy\n", sipa_eth->netdev->name, netid,
		state ? "DEV_ON" : "DEV_OFF");
	return 0;
}
...
static int sipa_eth_open(struct net_device *dev)
{
	...
	pr_info("dev 0x%p eth 0x%p open %s netid %d term %d mac_h %d\n", dev,
		sipa_eth, dev->name, pdata->netid, pdata->term_type,
		pdata->mac_h);
	ret = sipa_nic_open(pdata->term_type, pdata->netid, sipa_eth_notify_cb,
			    (void *)sipa_eth);
			     ...
			     struct device *dev)
{
	...
	switch (id) {
	case 0 ... 7:
		snprintf(pdata->name, IFNAMSIZ, "%s%d", SIPA_ETH_IFACE_PREF,
			 id);
		break;
	case 8 ... 11:
		snprintf(pdata->name, IFNAMSIZ, "%s%d", SIPA_ETH_VPCIE_PREF,
			 id - SIPA_ETH_VPCIE_IDX);
		break;
	default:
...
static DEVICE_ATTR(mode, 0664, sipa_eth_get_direct_mode,
		   sipa_eth_set_direct_mode);
...
static int sipa_eth_probe(struct platform_device *pdev)
{
	...
	netdev = alloc_netdev(sizeof(struct SIPA_ETH), ifname,
			      NET_NAME_PREDICTABLE, s_setup);
#else
	netdev = alloc_netdev(sizeof(struct SIPA_ETH), ifname, s_setup);
#endif
	...
	if (!strncmp(netdev->name, SIPA_ETH_VPCIE_PREF,
		     strlen(SIPA_ETH_VPCIE_PREF))) {
		...
		if (nss_cb) {
			...
			if (rc) {
				...
				netdev_err(netdev,
					   "Device will not use NSS path: %d\n",
					   rc);
			} else {
				netdev_info(netdev, "NSS context created\n");
				rtnl_lock();
				netdev_rx_handler_register(
					netdev, sipa_eth_rx_handler, NULL);
				rtnl_unlock();
			}
		} else {
			netdev_err(
				netdev,
				"nss_cb is null, Device will not use NSS path\n");
		}
#endif
...
static int sipa_eth_platform_device_reigster(void)
{
	...
	for (i = 0; i < SIPA_ETH_NUM; i++) {
		sipa_eth_pdev[i] =
			platform_device_alloc(SIPA_ETH_IFACE_PREF, i);
		if (!sipa_eth_pdev[i]) {
			i--;
			while (i >= 0)
	...
	for (i = 0; i < SIPA_ETH_NUM; i++) {
		sipa_eth_data[i] =
			kzalloc(sizeof(struct sipa_eth_init_data), GFP_KERNEL);
		if (!sipa_eth_data[i]) {
			retval = -ENOMEM;
			goto err_add_pdata;
		}
		...
		retval = platform_device_add_data(
			sipa_eth_pdev[i], sipa_eth_data[i],
...
static int sipa_eth_debug_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < SIPA_DUMMY_IFACE_NUM; i++) {
		...
		seq_printf(m, "dev %s, netid %d state %d\n",
			   dev_list[i]->ndev->name, dev_list[i]->netid,
			   dev_list[i]->state);
	}
...
static int sipa_eth_debugfs_mknod(void *root, void *data)
{
	...
	debugfs_create_file("stats", 0444, subroot, data, &sipa_eth_debug_fops);

	debugfs_create_file("gro_enable", 0600, (struct dentry *)root,
			    &gro_enable, &fops_gro_enable);
