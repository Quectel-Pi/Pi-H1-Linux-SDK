...
static int fxgmac_disable_rx_csum(struct fxgmac_pdata *pdata)
{
	...
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_IPC_POS, MAC_CR_IPC_LEN, 0);
...
static int fxgmac_enable_rx_csum(struct fxgmac_pdata *pdata)
{
	...
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_IPC_POS, MAC_CR_IPC_LEN, 1);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_CR);
			       ...
			       struct netdev_hw_addr *ha, unsigned int *mac_reg)
{
	...
	if (ha) {
		...
		mac_addr_hi = FXGMAC_SET_REG_BITS(
			mac_addr_hi, MAC_MACA1HR_AE_POS, MAC_MACA1HR_AE_LEN, 1);
	}
...
static int fxgmac_enable_rx_vlan_filtering(struct fxgmac_pdata *pdata)
{
	...
	regval = FXGMAC_SET_REG_BITS(regval, MAC_PFR_VTFE_POS, MAC_PFR_VTFE_LEN,
				     1);
				       ...
				       unsigned int enable)
{
	...
	if (FXGMAC_GET_REG_BITS(regval, MAC_PFR_PR_POS, MAC_PFR_PR_LEN) ==
	    val) {
		return 0;
	}
	netif_dbg(pdata, drv, pdata->netdev,
		  "" STR_FORMAT " promiscuous mode\n",
		  enable ? "entering" : "leaving");

	regval = FXGMAC_SET_REG_BITS(regval, MAC_PFR_PR_POS, MAC_PFR_PR_LEN,
				     val);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_PFR);

	DbgPrintF(MP_TRACE, "" STR_FORMAT " - promiscuous mode=%d, reg=%x.",
		  __FUNCTION__, enable, regval);
	DbgPrintF(
		MP_TRACE,
		"" STR_FORMAT
		" - note, vlan filter is called when set promiscuous mode=%d.",
		__FUNCTION__, enable);
...
static int fxgmac_enable_rx_broadcast(struct fxgmac_pdata *pdata,
				      unsigned int enable)
{
	/* mac reg bit is disable, so invert the val. */
	unsigned int val = enable ? 0 : 1;
	u32 regval;

	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_PFR);

	if (FXGMAC_GET_REG_BITS(regval, MAC_PFR_DBF_POS, MAC_PFR_DBF_LEN) ==
	    val) {
		return 0;
	}

	regval = FXGMAC_SET_REG_BITS(regval, MAC_PFR_DBF_POS, MAC_PFR_DBF_LEN,
				     val);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_PFR);

	DbgPrintF(MP_TRACE, "%s - bcast en=%d, bit-val=%d, reg=%x.",
		  __FUNCTION__, enable, val, regval);
	return 0;
}
					 ...
					 unsigned int enable)
{
	...
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_PFR);
	if (FXGMAC_GET_REG_BITS(regval, MAC_PFR_PM_POS, MAC_PFR_PM_LEN) ==
	    val) {
		return 0;
	}
	...
	regval = FXGMAC_SET_REG_BITS(regval, MAC_PFR_PM_POS, MAC_PFR_PM_LEN,
				     val);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_PFR);

	DbgPrintF(MP_TRACE,
		  "" STR_FORMAT " - Enable all Multicast=%d, regval=%#x.",
		  __FUNCTION__, enable, regval);
						   ...
						   int b_add)
{
	...
	if (!pmc_mac) {
		for (j = 0; j < FXGMAC_MAC_HASH_TABLE_SIZE; j++) {
			...
			writereg(pdata->pAdapter, 0,
				 pdata->mac_regs + hash_reg);
		}
		DBGPRINT(
			MP_TRACE,
			("<fxgmac_config_mcast_mac_hash_table, clear all mcast mac hash table\n"));
		return;
	}
...
static int fxgmac_config_jumbo(struct fxgmac_pdata *pdata)
{
	...
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_JE_POS, MAC_CR_JE_LEN,
				     pdata->jumbo);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_CR);
	return 0;
}
...
{
	fxgmac_disable_tx_vlan(
		pdata); /* configure dynamical vlanID from TX Context. */
				   ...
				   struct fxgmac_channel *channel)
{
	...
	while (time_before(jiffies, tx_timeout)) {
		...
		tx_status =
			FXGMAC_GET_REG_BITS(tx_status, tx_pos, DMA_DSR_TPS_LEN);
...
static void fxgmac_enable_tx(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->tx_q_count; i++) {
		regval = readreg(pdata->pAdapter,
				 FXGMAC_MTL_REG(pdata, i, MTL_Q_TQOMR));
...
static void fxgmac_disable_tx(struct fxgmac_pdata *pdata)
{
	...
	/* Disable MAC Tx */
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_TE_POS, MAC_CR_TE_LEN, 0);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_CR);
...
#if FXGMAC_TX_HANG_TIMER_EN
	rx_timeout =
		jiffies + msecs_to_jiffies(500); /* 500ms, larger is better */
#else
	rx_timeout = jiffies + (FXGMAC_DMA_STOP_TIMEOUT * HZ);
#endif
	while (time_before(jiffies, rx_timeout)) {
		rx_status = readreg(pdata->pAdapter,
				    FXGMAC_MTL_REG(pdata, queue, MTL_Q_RQDR));
...
static void fxgmac_enable_rx(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->channel_count; i++, channel++) {
		regval = readreg(pdata->pAdapter,
				 FXGMAC_DMA_REG(channel, DMA_CH_RCR));
	...
	/* Enable MAC Rx */
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_CST_POS, MAC_CR_CST_LEN, 1);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_ACS_POS, MAC_CR_ACS_LEN, 1);
...
static void fxgmac_enable_channel_rx(struct fxgmac_pdata *pdata,
				     unsigned int queue)
{
	struct fxgmac_channel *channel;
	unsigned int regval;
	...
	/* Enable MAC Rx */
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
	if (!(regval & ((0x01 << MAC_CR_CST_POS) | (0x01 << MAC_CR_ACS_POS) |
			(0x01 << MAC_CR_RE_POS)))) {
...
static void fxgmac_disable_rx(struct fxgmac_pdata *pdata)
{
	...
	/* Disable MAC Rx */
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_CST_POS, MAC_CR_CST_LEN, 0);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_ACS_POS, MAC_CR_ACS_LEN, 0);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_RE_POS, MAC_CR_RE_LEN, 0);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_CR);
...
static int fxgmac_is_context_desc(struct fxgmac_dma_desc *dma_desc)
{
	...
	return FXGMAC_GET_REG_BITS_LE(dma_desc->desc3, TX_NORMAL_DESC3_CTXT_POS,
...
static int fxgmac_is_last_desc(struct fxgmac_dma_desc *dma_desc)
{
	...
	return FXGMAC_GET_REG_BITS_LE(dma_desc->desc3, TX_NORMAL_DESC3_LD_POS,
...
static int fxgmac_disable_tx_flow_control(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->rx_q_count; i++) {
		regval = readreg(pdata->pAdapter,
				 FXGMAC_MTL_REG(pdata, i, MTL_Q_RQOMR));
...
static int fxgmac_disable_rx_flow_control(struct fxgmac_pdata *pdata)
{
	...
	regval = FXGMAC_SET_REG_BITS(regval, MAC_RFCR_RFE_POS, MAC_RFCR_RFE_LEN,
				     0);
...
static int fxgmac_config_rx_coalesce(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->channel_count; i++, channel++) {
		...
		regval = readreg(pdata->pAdapter,
				 FXGMAC_DMA_REG(channel, DMA_CH_RIWT));
...
static void fxgmac_config_rx_fep_disable(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->rx_q_count; i++) {
		regval = readreg(pdata->pAdapter,
				 FXGMAC_MTL_REG(pdata, i, MTL_Q_RQOMR));
		/* 1:enable  the  rx queue forward packet with error
		 * status(crc error, gmii_er, watch dog timeout.or overflow)
		 */
...
static void fxgmac_config_sph_mode(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->channel_count; i++, channel++) {
		...
		regval = readreg(pdata->pAdapter,
				 FXGMAC_DMA_REG(channel, DMA_CH_CR));
	...
	regval = FXGMAC_SET_REG_BITS(regval, MAC_ECR_HDSMS_POS,
				     MAC_ECR_HDSMS_LEN, FXGMAC_SPH_HDSMS_SIZE);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_ECR);
}
...
static void fxgmac_config_mtl_mode(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->tx_q_count /*hw_feat.tc_cnt*/; i++) {
		regval = readreg(pdata->pAdapter,
				 FXGMAC_MTL_REG(pdata, i, MTL_TC_QWR));
	...
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MTL_OMR);
	regval = FXGMAC_SET_REG_BITS(regval, MTL_OMR_RAA_POS, MTL_OMR_RAA_LEN,
				     MTL_RAA_SP);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MTL_OMR);
}
...
static void fxgmac_config_flow_control_threshold(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->rx_q_count; i++) {
		...
		regval = FXGMAC_SET_REG_BITS(regval, MTL_Q_RQOMR_RFA_POS,
					     MTL_Q_RQOMR_RFA_LEN, 6);
		/* De-activate flow control when more than 10k left in fifo */
		regval = FXGMAC_SET_REG_BITS(regval, MTL_Q_RQOMR_RFD_POS,
					     MTL_Q_RQOMR_RFD_LEN, 10);
		writereg(pdata->pAdapter, regval,
			 FXGMAC_MTL_REG(pdata, i, MTL_Q_RQOMR));
	}
}
...
static int fxgmac_config_rsf_mode(struct fxgmac_pdata *pdata, unsigned int val)
{
	unsigned int i;
	u32 regval;
...
static int fxgmac_config_tsf_mode(struct fxgmac_pdata *pdata, unsigned int val)
{
	unsigned int i;
	u32 regval;
...
static int fxgmac_config_pblx8(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->channel_count; i++, channel++) {
		...
		regval = FXGMAC_SET_REG_BITS(regval, DMA_CH_CR_PBLX8_POS,
					     DMA_CH_CR_PBLX8_LEN, pdata->pblx8);
		writereg(pdata->pAdapter, regval,
			 FXGMAC_DMA_REG(channel, DMA_CH_CR));
	}
#else
...
static int fxgmac_get_tx_pbl_val(struct fxgmac_pdata *pdata)
{
	...
	regval = readreg(pdata->pAdapter,
			 FXGMAC_DMA_REG(pdata->channel_head, DMA_CH_TCR));
...
static int fxgmac_config_tx_pbl_val(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->channel_count; i++, channel++) {
		...
		regval = FXGMAC_SET_REG_BITS(regval, DMA_CH_TCR_PBL_POS,
					     DMA_CH_TCR_PBL_LEN, pdata->tx_pbl);
		writereg(pdata->pAdapter, regval,
			 FXGMAC_DMA_REG(channel, DMA_CH_TCR));
	}
#else
...
static int fxgmac_get_rx_pbl_val(struct fxgmac_pdata *pdata)
{
	...
	regval = readreg(pdata->pAdapter,
			 FXGMAC_DMA_REG(pdata->channel_head, DMA_CH_RCR));
...
static int fxgmac_config_rx_pbl_val(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->channel_count; i++, channel++) {
		...
		regval = FXGMAC_SET_REG_BITS(regval, DMA_CH_RCR_PBL_POS,
					     DMA_CH_RCR_PBL_LEN, pdata->rx_pbl);
		writereg(pdata->pAdapter, regval,
			 FXGMAC_DMA_REG(channel, DMA_CH_RCR));
	}
#else
...
static void fxgmac_tx_mmc_int(struct fxgmac_pdata *pdata)
{
	...
	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_TISR_TXCARRIERERRORFRAMES_POS,
				MMC_TISR_TXCARRIERERRORFRAMES_LEN))
		stats->txcarriererrorframes +=
			fxgmac_mmc_read(pdata, MMC_TXCARRIERERRORFRAMES);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_TISR_TXOCTETCOUNT_G_POS,
...
static void fxgmac_rx_mmc_int(struct fxgmac_pdata *pdata)
{
	...
	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXFRAMECOUNT_GB_POS,
				MMC_RISR_RXFRAMECOUNT_GB_LEN))
		stats->rxframecount_gb +=
			fxgmac_mmc_read(pdata, MMC_RXFRAMECOUNT_GB_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXOCTETCOUNT_GB_POS,
				MMC_RISR_RXOCTETCOUNT_GB_LEN))
		stats->rxoctetcount_gb +=
			fxgmac_mmc_read(pdata, MMC_RXOCTETCOUNT_GB_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXOCTETCOUNT_G_POS,
				MMC_RISR_RXOCTETCOUNT_G_LEN))
		stats->rxoctetcount_g +=
			fxgmac_mmc_read(pdata, MMC_RXOCTETCOUNT_G_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXBROADCASTFRAMES_G_POS,
				MMC_RISR_RXBROADCASTFRAMES_G_LEN))
		stats->rxbroadcastframes_g +=
			fxgmac_mmc_read(pdata, MMC_RXBROADCASTFRAMES_G_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXMULTICASTFRAMES_G_POS,
				MMC_RISR_RXMULTICASTFRAMES_G_LEN))
		stats->rxmulticastframes_g +=
			fxgmac_mmc_read(pdata, MMC_RXMULTICASTFRAMES_G_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXCRCERROR_POS,
				MMC_RISR_RXCRCERROR_LEN))
		stats->rxcrcerror += fxgmac_mmc_read(pdata, MMC_RXCRCERROR_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXALIGNERROR_POS,
				MMC_RISR_RXALIGNERROR_LEN))
		stats->rxalignerror += fxgmac_mmc_read(pdata, MMC_RXALIGNERROR);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXRUNTERROR_POS,
				MMC_RISR_RXRUNTERROR_LEN))
		stats->rxrunterror += fxgmac_mmc_read(pdata, MMC_RXRUNTERROR);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXJABBERERROR_POS,
				MMC_RISR_RXJABBERERROR_LEN))
		stats->rxjabbererror +=
			fxgmac_mmc_read(pdata, MMC_RXJABBERERROR);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXUNDERSIZE_G_POS,
				MMC_RISR_RXUNDERSIZE_G_LEN))
		stats->rxundersize_g +=
			fxgmac_mmc_read(pdata, MMC_RXUNDERSIZE_G);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXOVERSIZE_G_POS,
				MMC_RISR_RXOVERSIZE_G_LEN))
		stats->rxoversize_g += fxgmac_mmc_read(pdata, MMC_RXOVERSIZE_G);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RX64OCTETS_GB_POS,
				MMC_RISR_RX64OCTETS_GB_LEN))
		stats->rx64octets_gb +=
			fxgmac_mmc_read(pdata, MMC_RX64OCTETS_GB_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RX65TO127OCTETS_GB_POS,
				MMC_RISR_RX65TO127OCTETS_GB_LEN))
		stats->rx65to127octets_gb +=
			fxgmac_mmc_read(pdata, MMC_RX65TO127OCTETS_GB_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RX128TO255OCTETS_GB_POS,
				MMC_RISR_RX128TO255OCTETS_GB_LEN))
		stats->rx128to255octets_gb +=
			fxgmac_mmc_read(pdata, MMC_RX128TO255OCTETS_GB_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RX256TO511OCTETS_GB_POS,
				MMC_RISR_RX256TO511OCTETS_GB_LEN))
		stats->rx256to511octets_gb +=
			fxgmac_mmc_read(pdata, MMC_RX256TO511OCTETS_GB_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RX512TO1023OCTETS_GB_POS,
				MMC_RISR_RX512TO1023OCTETS_GB_LEN))
		stats->rx512to1023octets_gb +=
			fxgmac_mmc_read(pdata, MMC_RX512TO1023OCTETS_GB_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RX1024TOMAXOCTETS_GB_POS,
				MMC_RISR_RX1024TOMAXOCTETS_GB_LEN))
		stats->rx1024tomaxoctets_gb +=
			fxgmac_mmc_read(pdata, MMC_RX1024TOMAXOCTETS_GB_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXUNICASTFRAMES_G_POS,
				MMC_RISR_RXUNICASTFRAMES_G_LEN))
		stats->rxunicastframes_g +=
			fxgmac_mmc_read(pdata, MMC_RXUNICASTFRAMES_G_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXLENGTHERROR_POS,
				MMC_RISR_RXLENGTHERROR_LEN))
		stats->rxlengtherror +=
			fxgmac_mmc_read(pdata, MMC_RXLENGTHERROR_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXOUTOFRANGETYPE_POS,
				MMC_RISR_RXOUTOFRANGETYPE_LEN))
		stats->rxoutofrangetype +=
			fxgmac_mmc_read(pdata, MMC_RXOUTOFRANGETYPE_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXPAUSEFRAMES_POS,
				MMC_RISR_RXPAUSEFRAMES_LEN))
		stats->rxpauseframes +=
			fxgmac_mmc_read(pdata, MMC_RXPAUSEFRAMES_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXFIFOOVERFLOW_POS,
				MMC_RISR_RXFIFOOVERFLOW_LEN))
		stats->rxfifooverflow +=
			fxgmac_mmc_read(pdata, MMC_RXFIFOOVERFLOW_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXVLANFRAMES_GB_POS,
				MMC_RISR_RXVLANFRAMES_GB_LEN))
		stats->rxvlanframes_gb +=
			fxgmac_mmc_read(pdata, MMC_RXVLANFRAMES_GB_LO);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXWATCHDOGERROR_POS,
				MMC_RISR_RXWATCHDOGERROR_LEN))
		stats->rxwatchdogerror +=
			fxgmac_mmc_read(pdata, MMC_RXWATCHDOGERROR);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXERRORFRAMES_POS,
				MMC_RISR_RXERRORFRAMES_LEN))
		stats->rxreceiveerrorframe +=
			fxgmac_mmc_read(pdata, MMC_RXRECEIVEERRORFRAME);

	if (FXGMAC_GET_REG_BITS(mmc_isr, MMC_RISR_RXERRORCONTROLFRAMES_POS,
...
static void fxgmac_read_mmc_stats(struct fxgmac_pdata *pdata)
{
	...
	regval = FXGMAC_SET_REG_BITS(regval, MMC_CR_MCF_POS, MMC_CR_MCF_LEN, 1);
	...
	stats->tx64octets_gb += fxgmac_mmc_read(pdata, MMC_TX64OCTETS_GB_LO);
	...
	stats->txdeferredframes += fxgmac_mmc_read(pdata, MMC_TXDEFERREDFRAMES);
	...
	stats->txoctetcount_g += fxgmac_mmc_read(pdata, MMC_TXOCTETCOUNT_G_LO);

	stats->txframecount_g += fxgmac_mmc_read(pdata, MMC_TXFRAMECOUNT_G_LO);

	stats->txexcessivedeferralerror +=
		fxgmac_mmc_read(pdata, MMC_TXEXCESSIVEDEFERRALERROR);

	stats->txpauseframes += fxgmac_mmc_read(pdata, MMC_TXPAUSEFRAMES_LO);

	stats->txvlanframes_g += fxgmac_mmc_read(pdata, MMC_TXVLANFRAMES_G_LO);

	stats->txoversize_g += fxgmac_mmc_read(pdata, MMC_TXOVERSIZEFRAMES);
	...
	stats->rxoctetcount_g += fxgmac_mmc_read(pdata, MMC_RXOCTETCOUNT_G_LO);
	...
	stats->rxjabbererror += fxgmac_mmc_read(pdata, MMC_RXJABBERERROR);

	stats->rxundersize_g += fxgmac_mmc_read(pdata, MMC_RXUNDERSIZE_G);

	stats->rxoversize_g += fxgmac_mmc_read(pdata, MMC_RXOVERSIZE_G);

	stats->rx64octets_gb += fxgmac_mmc_read(pdata, MMC_RX64OCTETS_GB_LO);
	...
	stats->rxlengtherror += fxgmac_mmc_read(pdata, MMC_RXLENGTHERROR_LO);

	stats->rxoutofrangetype +=
		fxgmac_mmc_read(pdata, MMC_RXOUTOFRANGETYPE_LO);

	stats->rxpauseframes += fxgmac_mmc_read(pdata, MMC_RXPAUSEFRAMES_LO);

	stats->rxfifooverflow += fxgmac_mmc_read(pdata, MMC_RXFIFOOVERFLOW_LO);

	stats->rxvlanframes_gb +=
		fxgmac_mmc_read(pdata, MMC_RXVLANFRAMES_GB_LO);

	stats->rxwatchdogerror += fxgmac_mmc_read(pdata, MMC_RXWATCHDOGERROR);

	stats->rxreceiveerrorframe +=
		fxgmac_mmc_read(pdata, MMC_RXRECEIVEERRORFRAME);

	stats->rxcontrolframe_g += fxgmac_mmc_read(pdata, MMC_RXCONTROLFRAME_G);

	/* Un-freeze counters */
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MMC_CR);
	regval = FXGMAC_SET_REG_BITS(regval, MMC_CR_MCF_POS, MMC_CR_MCF_LEN, 0);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MMC_CR);
}
...
static void fxgmac_config_mmc(struct fxgmac_pdata *pdata)
{
	...
	regval = FXGMAC_SET_REG_BITS(regval, MMC_CR_ROR_POS, MMC_CR_ROR_LEN, 1);
	/* Reset the counters */
...
static u32 fxgmac_read_rss_options(struct fxgmac_pdata *pdata)
{
	...
	regval = FXGMAC_GET_REG_BITS(regval, MGMT_RSS_CTRL_OPT_POS,
...
static int fxgmac_read_rss_hash_key(struct fxgmac_pdata *pdata, u8 *key_buf)
{
	...
	while (key_regs--) {
		(*key) = cpu_to_be32(readreg(
			pdata->pAdapter,
			pdata->base_mem + (MGMT_RSS_KEY0 +
					   key_regs * MGMT_RSS_KEY_REG_INC)));

		DBGPRINT(
			MP_LOUD,
			("fxgmac_read_rss_hash_key: idx=%d, reg=%x, key=0x%08x\n",
			 key_regs,
			 MGMT_RSS_KEY0 + key_regs * MGMT_RSS_KEY_REG_INC,
			 (u32)(*key)));
		key++;
	}
...
static int fxgmac_write_rss_hash_key(struct fxgmac_pdata *pdata)
{
	...
	while (key_regs--) {
		ret = fxgmac_write_rss_reg(
			pdata, FXGMAC_RSS_HASH_KEY_TYPE,
			MGMT_RSS_KEY0 + key_regs * MGMT_RSS_KEY_REG_INC,
			cpu_to_be32(*key));
		if (ret)
			return ret;
...
static int fxgmac_write_rss_lookup_table(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0, j = 0; i < ARRAY_SIZE(pdata->rss_table); i++, j++) {
		if (j < MGMT_RSS_IDT_ENTRY_PER_REG) {
			regval |=
				((pdata->rss_table[i] & MGMT_RSS_IDT_ENTRY_MASK)
				 << (j * 2));
		} else {
			ret = fxgmac_write_rss_reg(
				pdata, FXGMAC_RSS_LOOKUP_TABLE_TYPE,
				MGMT_RSS_IDT + (i / MGMT_RSS_IDT_ENTRY_PER_REG -
						1) * MGMT_RSS_IDT_REG_INC,
				regval);
			if (ret)
				return ret;
	...
	if (j == MGMT_RSS_IDT_ENTRY_PER_REG) {
		/* last IDT */
		fxgmac_write_rss_reg(
			pdata, FXGMAC_RSS_LOOKUP_TABLE_TYPE,
			MGMT_RSS_IDT + (i / MGMT_RSS_IDT_ENTRY_PER_REG - 1) *
					       MGMT_RSS_IDT_REG_INC,
			regval);
	}
...
static int fxgmac_enable_rss(struct fxgmac_pdata *pdata)
{
	...
	regval = FXGMAC_SET_REG_BITS(regval, 8, 2, 1);
#endif
	/* Enable RSS */
...
static void fxgmac_config_rss(struct fxgmac_pdata *pdata)
{
	...
	if (ret) {
		DBGPRINT(MP_ERROR,
			 ("fxgmac_config_rss: error configuring RSS\n"));
	}
}
...
static void fxgmac_update_aoe_ipv4addr(struct fxgmac_pdata *pdata, u8 *ip_addr)
{
	...
	if (regval != /*cpu_to_be32*/ (ipval)) {
		writereg(pdata->pAdapter, /*cpu_to_be32*/ (ipval),
			 pdata->mac_regs + MAC_ARP_PROTO_ADDR);
...
static int fxgmac_enable_arp_offload(struct fxgmac_pdata *pdata)
{
	...
	/* Enable arpoffload */
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_ARPEN_POS, MAC_CR_ARPEN_LEN,
				     1);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_CR);
 ...
 */
static int fxgmac_set_ns_offload(struct fxgmac_pdata *pdata, unsigned int index,
				 ...
				 unsigned char *mac_addr)
{
	...
	regval = readreg(pdata->pAdapter,
			 pdata->base_mem + 0X38 * index + NS_LUT_MAC_ADDR_CTL);
	...
	for (i = 0; i < 16 / 4; i++) {
		Address[i] = (remote_addr[i * 4 + 0] << 24) |
			     ...
			     (remote_addr[i * 4 + 2] << 8) |
			     (remote_addr[i * 4 + 3] << 0);
		writereg(pdata->pAdapter, Address[i],
			 pdata->base_mem + 0X38 * index + NS_LUT_ROMOTE0 +
				 4 * i);
		if (Address[i]) {
			remote_not_zero = 1;
		}
		...
		writereg(pdata->pAdapter, Address[i],
			 pdata->base_mem + 0X38 * index + NS_LUT_TARGET0 +
				 4 * i);
		...
		writereg(pdata->pAdapter, Address[i],
			 pdata->base_mem + 0X38 * index + NS_LUT_SOLICITED0 +
				 4 * i);
		...
		writereg(pdata->pAdapter, Address[i],
			 pdata->base_mem + 0X10 * index + NS_LUT_TARGET4 +
				 4 * i);
	}
	...
	writereg(pdata->pAdapter, mac_addr_hi,
		 pdata->base_mem + 0X38 * index + NS_LUT_MAC_ADDR);
	if (remote_not_zero == 0) {
		regval = readreg(pdata->pAdapter, pdata->base_mem +
							  0X38 * index +
							  NS_LUT_MAC_ADDR_CTL);
		regval = FXGMAC_SET_REG_BITS(regval, NS_LUT_REMOTE_AWARED_POS,
					     NS_LUT_REMOTE_AWARED_LEN, 0);
		regval = FXGMAC_SET_REG_BITS(regval, NS_LUT_MAC_ADDR_LOW_POS,
					     NS_LUT_MAC_ADDR_LOW_LEN,
					     mac_addr_lo);
		writereg(pdata->pAdapter, regval,
			 pdata->base_mem + 0X38 * index + NS_LUT_MAC_ADDR_CTL);
	} else {
					      ...
					      unsigned int param)
{
	...
	if (NULL == fxgmac_get_netdev_ip6addr(pdata, target_addr1,
					      solicited_addr,
					      (FXGMAC_NS_IFA_LOCAL_LINK |
					       FXGMAC_NS_IFA_GLOBAL_UNICAST) &
						      param)) {
	...
	DPRINTK("%s, Get net device MAC addr ok, ns_tab idx=%d, %02x:%02x:%02x:%02x:%02x:%02x\n",
		__FUNCTION__, pdata->expansion.ns_offload_tab_idx, mac_addr[0],
...
static int fxgmac_check_wake_pattern_fifo_pointer(struct fxgmac_pdata *pdata)
{
	...
	regval = FXGMAC_SET_REG_BITS(regval, MAC_PMT_STA_RWKFILTERST_POS,
				     MAC_PMT_STA_RWKFILTERST_LEN, 1);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_PMT_STA);

	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_PMT_STA);
	regval = FXGMAC_GET_REG_BITS(regval, MAC_PMT_STA_RWKPTR_POS,
					...
					u32 Data)
{
	const u16 address_offset[16][3] = {
		{ 0x1020, 0x1024, 0x1028 }, { 0x102c, 0x1030, 0x1034 },
		{ 0x1038, 0x103c, 0x1040 }, { 0x1044, 0x1050, 0x1054 },
		{ 0x1058, 0x105c, 0x1060 }, { 0x1064, 0x1068, 0x106c },
		{ 0x1070, 0x1074, 0x1078 }, { 0x107c, 0x1080, 0x1084 },
		{ 0x1088, 0x108c, 0x1090 }, { 0x1134, 0x113c, 0x1140 },
		{ 0x1208, 0x1200, 0x1204 }, { 0x1218, 0x1210, 0x1214 },
		{ 0x1228, 0x1220, 0x1224 }, { 0x1238, 0x1230, 0x1234 },
		{ 0x1248, 0x1240, 0x1244 }, { 0x1258, 0x1250, 0x1254 },
	};
	if (filter_index > 15 || register_index > 2) {
		DbgPrintF(
			MP_TRACE,
			"%s - Remote mask  pointer  is  over range, filter_index:%d, register_index:0x%x\n",
			__FUNCTION__, filter_index, register_index);
		return -1;
	}
	writereg(pdata->pAdapter, Data,
		 pdata->base_mem +
			 address_offset[filter_index][register_index]);
	return 0;
}
				   ...
				   u32 pattern_cnt)
{
	...
	if (pattern_cnt > MAX_PATTERN_COUNT) {
		DbgPrintF(
			MP_TRACE,
			"%s - Error: %d patterns, exceed %d, not supported!\n",
	...
	if (fxgmac_check_wake_pattern_fifo_pointer(pdata)) {
		DbgPrintF(
			MP_TRACE,
			"%s - Warning: the remote pattern array pointer is not be 0\n",
			__FUNCTION__);
		return -1;
	}

	for (i = 0; i < pattern_cnt; i++) {
		memcpy(&pdata->pattern[i], wol_pattern + i,
		...
		if (pattern_cnt + pattern_inherited_cnt < MAX_PATTERN_COUNT) {
			if (wol_pattern[i].pattern_offset ||
				...
				memcpy(&pdata->pattern[pattern_cnt +
						       pattern_inherited_cnt],
				       wol_pattern + i, sizeof(wol_pattern[0]));
				pattern_inherited_cnt++;
			}
		}
	}
	...
	for (i = 0; i < total_cnt; i++) {
		...
		for (j = 0; j < pdata->pattern[i].mask_size; j++) {
			for (z = 0;
			     ...
			     z++) {
				if (pdata->pattern[i].mask_info[j] &
				    (0x01 << z)) {
					mask[map_index] =
						pdata->pattern[i].pattern_info
							[pdata->pattern[i]
								 .pattern_offset +
							 mask_index];
					map_index++;
		...
		for (kp = 0, km = 0;
		     kp < sizeof(pdata->pattern[i].pattern_info);
		     kp += 16, km += 2) {
		...
		DbgPrintF(
			MP_LOUD,
			"WritePatternToNic62 the %d patterns crc = %x mask length = %d, mask_offset=%x.\n",
	...
	for (i = 0; i < (total_cnt + 3) / 4; i++) {
		...
		writereg(pdata->pAdapter,
			 ((pdata->pattern[i * 4 + 0].mask_info[3] & 0x7f)
			  << 24) |
				 (pdata->pattern[i * 4 + 0].mask_info[2]
				  << 16) |
		...
		writereg(pdata->pAdapter,
			 ((pdata->pattern[i * 4 + 1].mask_info[3] & 0x7f)
			  << 24) |
				 (pdata->pattern[i * 4 + 1].mask_info[2]
				  << 16) |
		...
		writereg(pdata->pAdapter,
			 ((pdata->pattern[i * 4 + 2].mask_info[3] & 0x7f)
			  << 24) |
				 (pdata->pattern[i * 4 + 2].mask_info[2]
				  << 16) |
		...
		writereg(pdata->pAdapter,
			 ((pdata->pattern[i * 4 + 3].mask_info[3] & 0x7f)
			  << 24) |
				 (pdata->pattern[i * 4 + 3].mask_info[2]
				  << 16) |
		...
		regval |= (i * 4 + 0 >= total_cnt)   ? 0x0 :
			  ...
			  pdata->pattern[i * 4 + 0].pattern_offset ?
						       0x0 :
			  !(pdata->pattern[i * 4 + 0].mask_info[0] & 0x01) ?
						       0x0 :
			  (pdata->pattern[i * 4 + 0].pattern_info[0] & 0x01) ?
						       (0x1 << (3 + 0)) :
						       0x0;
		regval |= (i * 4 + 1 >= total_cnt)   ? 0x0 :
			  (i * 4 + 1 >= pattern_cnt) ? (0x1 << (3 + 8)) :
			  pdata->pattern[i * 4 + 1].pattern_offset ?
						       0x0 :
			  !(pdata->pattern[i * 4 + 1].mask_info[0] & 0x01) ?
						       0x0 :
			  (pdata->pattern[i * 4 + 1].pattern_info[0] & 0x01) ?
						       (0x1 << (3 + 8)) :
						       0x0;
		regval |= (i * 4 + 2 >= total_cnt)   ? 0x0 :
			  (i * 4 + 2 >= pattern_cnt) ? (0x1 << (3 + 16)) :
			  pdata->pattern[i * 4 + 2].pattern_offset ?
						       0x0 :
			  !(pdata->pattern[i * 4 + 2].mask_info[0] & 0x01) ?
						       0x0 :
			  (pdata->pattern[i * 4 + 2].pattern_info[0] & 0x01) ?
						       (0x1 << (3 + 16)) :
						       0x0;
		regval |= (i * 4 + 3 >= total_cnt)   ? 0x0 :
			  (i * 4 + 3 >= pattern_cnt) ? (0x1 << (3 + 24)) :
			  pdata->pattern[i * 4 + 3].pattern_offset ?
						       0x0 :
			  !(pdata->pattern[i * 4 + 3].mask_info[0] & 0x01) ?
						       0x0 :
			  (pdata->pattern[i * 4 + 3].pattern_info[0] & 0x01) ?
						       (0x1 << (3 + 24)) :
						       0x0;
		writereg(pdata->pAdapter, regval,
			 pdata->mac_regs + MAC_RWK_PAC);
	...
	for (i = 0; i < total_cnt; i++) {
		fxgmac_set_wake_pattern_mask(
			...
			((pdata->pattern[i].mask_info[7] & 0x7f) << (24 + 1)) |
				...
				((pdata->pattern[i].mask_info[3] & 0x80) >>
				 7)); /* global  manager  regitster  mask bit  31~62 */
		fxgmac_set_wake_pattern_mask(
			pdata, i, 1,
			((pdata->pattern[i].mask_info[11] & 0x7f) << (24 + 1)) |
				...
				((pdata->pattern[i].mask_info[7] & 0x80) >>
				 7)); /* global  manager  regitster  mask bit  63~94 */
		fxgmac_set_wake_pattern_mask(
			pdata, i, 2,
			((pdata->pattern[i].mask_info[15] & 0x7f) << (24 + 1)) |
				...
				((pdata->pattern[i].mask_info[11] & 0x80) >>
				 7)); /* global  manager  regitster  mask bit  95~126 */
	}
...
static int fxgmac_enable_wake_pattern(struct fxgmac_pdata *pdata)
{
	...
	regval = FXGMAC_SET_REG_BITS(regval, MAC_PMT_STA_RWKFILTERST_POS,
				     MAC_PMT_STA_RWKFILTERST_LEN, 1);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_PMT_STA_RWKPKTEN_POS,
...
static int fxgmac_enable_wake_magic_pattern(struct fxgmac_pdata *pdata)
{
	...
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_PMT_STA);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_PMT_STA_MGKPKTEN_POS,
	...
	regval = FXGMAC_SET_REG_BITS(regval, PM_CTRLSTAT_PME_EN_POS,
				     PM_CTRLSTAT_PME_EN_LEN, 1);
						 ...
						 int en)
{
	...
	val_wpi_crtl0 =
		(u32)readreg(pdata->pAdapter, pdata->base_mem + MGMT_WPI_CTRL0);

	/* prepare to write packet data by write wpi_mode to 1 */
	val_wpi_crtl0 =
		FXGMAC_SET_REG_BITS(val_wpi_crtl0, MGMT_WPI_CTRL0_WPI_MODE_POS,
				    MGMT_WPI_CTRL0_WPI_MODE_LEN,
				    (en ? MGMT_WPI_CTRL0_WPI_MODE_WR :
					  MGMT_WPI_CTRL0_WPI_MODE_NORMAL));
	writereg(pdata->pAdapter, val_wpi_crtl0,
		 pdata->base_mem + MGMT_WPI_CTRL0);
 ...
 *
 */
static void fxgmac_get_wake_packet_indication(struct fxgmac_pdata *pdata,
					      int *wake_reason,
					      ...
					      u32 *packet_size)
{
	...
	*packet_size = FXGMAC_GET_REG_BITS(val_wpi_crtl0,
	...
	if (buf_size < *packet_size) {
		DbgPrintF(MP_WARN,
			  "%s - too small buf_size=%u, WPI pkt len is %u.\n",
	...
	while ((0 == (val_wpi_crtl0 & MGMT_WPI_CTRL0_WPI_OP_DONE))) {
		...
		val_wpi_crtl0 = (u32)readreg(pdata->pAdapter,
					     pdata->base_mem + MGMT_WPI_CTRL0);
		i++;
	}
	if (*packet_size <= MAC_CRC_LENGTH) {
		DbgPrintF(MP_TRACE,
			  "%s - Warning, WPI pkt len is less 4 from hw.\n",
			  __FUNCTION__);
		return;
	}
	*packet_size -= MAC_CRC_LENGTH;
	...
	DbgPrintF(
		MP_TRACE,
		"%s - WPI done and back to normal mode, reg=%08x, read data=%dB.\n",
...
static int fxgmac_enable_wake_link_change(struct fxgmac_pdata *pdata)
{
	...
	regval = readreg(pdata->pAdapter, pdata->base_mem + WOL_CTL);
	regval = FXGMAC_SET_REG_BITS(regval, WOL_LINKCHG_EN_POS,
				     WOL_LINKCHG_EN_LEN, 1);
	writereg(pdata->pAdapter, regval, pdata->base_mem + WOL_CTL);
	return 0;
}
...
static void fxgmac_enable_dma_interrupts(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < pdata->channel_count; i++, channel++) {
		...
		dma_ch_isr = readreg(pdata->pAdapter,
				     FXGMAC_DMA_REG(channel, DMA_CH_SR));
		writereg(pdata->pAdapter, dma_ch_isr,
			 FXGMAC_DMA_REG(channel, DMA_CH_SR));
		 ...
		 */
		dma_ch_ier = FXGMAC_SET_REG_BITS(dma_ch_ier, DMA_CH_IER_NIE_POS,
						 DMA_CH_IER_NIE_LEN, 1);
		/* dma_ch_ier = FXGMAC_SET_REG_BITS(dma_ch_ier,
		 * DMA_CH_IER_AIE_POS, DMA_CH_IER_AIE_LEN, 1);
		 */
		dma_ch_ier = FXGMAC_SET_REG_BITS(dma_ch_ier,
						 DMA_CH_IER_FBEE_POS,
		...
		writereg(pdata->pAdapter, dma_ch_ier,
...
static void fxgmac_enable_mtl_interrupts(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0; i < q_count; i++) {
		/* Clear all the interrupts which are set */
		mtl_q_isr = readreg(pdata->pAdapter,
				    FXGMAC_MTL_REG(pdata, i, MTL_Q_ISR));
		writereg(pdata->pAdapter, mtl_q_isr,
			 FXGMAC_MTL_REG(pdata, i, MTL_Q_ISR));

		/* No MTL interrupts to be enabled */
		writereg(pdata->pAdapter, 0,
			 FXGMAC_MTL_REG(pdata, i, MTL_Q_IER));
	}
}
...
static int fxgmac_set_fxgmii_2500_speed(struct fxgmac_pdata *pdata)
{
	...
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_PS_POS, MAC_CR_PS_LEN, 0);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_FES_POS, MAC_CR_FES_LEN, 1);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_DM_POS, MAC_CR_DM_LEN,
				     pdata->phy_duplex);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_CR);
...
static int fxgmac_set_fxgmii_1000_speed(struct fxgmac_pdata *pdata)
{
	...
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_PS_POS, MAC_CR_PS_LEN, 0);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_FES_POS, MAC_CR_FES_LEN, 0);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_DM_POS, MAC_CR_DM_LEN,
				     pdata->phy_duplex);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_CR);
...
static int fxgmac_set_fxgmii_100_speed(struct fxgmac_pdata *pdata)
{
	...
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_CR);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_PS_POS, MAC_CR_PS_LEN, 1);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_FES_POS, MAC_CR_FES_LEN, 1);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_CR_DM_POS, MAC_CR_DM_LEN,
				     pdata->phy_duplex);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_CR);
 ...
 **/
static int fxgmac_check_phy_link(struct fxgmac_pdata *pdata, u32 *speed,
	...
	if (netdev->base_addr) {
		...
		*link_up = false;
		if (link_reg & MGMT_EPHY_CTRL_STA_EPHY_RELEASE) {
			if (link_up) {
				*link_up = (link_reg &
					    MGMT_EPHY_CTRL_STA_EPHY_LINKUP) ?
						   true :
						   false;
			}
			if (speed)
				*speed = (link_reg &
					  MGMT_EPHY_CTRL_STA_SPEED_MASK) >>
...
static int fxgmac_write_ephy_reg(struct fxgmac_pdata *pdata, u32 reg_id,
				 u32 data)
{
	u32 regval;
	...
	do {
		regval = readreg(pdata->pAdapter,
				 pdata->mac_regs + MAC_MDIO_ADDRESS);
		busy--;
...
static int fxgmac_read_ephy_reg(struct fxgmac_pdata *pdata, u32 reg_id,
				u32 *data)
{
	u32 regval = 0, regret;
	...
	if (0 == (regval & MAC_MDIO_ADDRESS_BUSY)) {
		regret = readreg(pdata->pAdapter,
				 pdata->mac_regs + MAC_MDIO_DATA);
		if (data)
			*data = regret;
		return regret;
	}
...
static int fxgmac_write_ephy_mmd_reg(struct fxgmac_pdata *pdata, u32 reg_id,
				     u32 mmd, u32 data)
{
	u32 regval;
...
static void fxgmac_config_flow_control(struct fxgmac_pdata *pdata)
{
	...
	if (pdata->tx_pause || pdata->rx_pause) {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_ADVERTISE_PAUSE_POS,
					     PHY_MII_ADVERTISE_PAUSE_LEN, 1);
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_ADVERTISE_ASYPAUSE_POS,
					     PHY_MII_ADVERTISE_ASYPAUSE_LEN, 1);
	} else {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_ADVERTISE_PAUSE_POS,
					     PHY_MII_ADVERTISE_PAUSE_LEN, 0);
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_ADVERTISE_ASYPAUSE_POS,
					     PHY_MII_ADVERTISE_ASYPAUSE_LEN, 0);
	}
					     ...
					     struct fxphy_ag_adv phy_ag_adv)
{
	...
	if (phy_ag_adv.full_1000m) {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_CTRL1000_1000FULL_POS,
					     PHY_MII_CTRL1000_1000FULL_LEN, 1);
	} else {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_CTRL1000_1000FULL_POS,
					     PHY_MII_CTRL1000_1000FULL_LEN, 0);
	}
	if (phy_ag_adv.half_1000m) {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_CTRL1000_1000HALF_POS,
					     PHY_MII_CTRL1000_1000HALF_LEN, 1);
	} else {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_CTRL1000_1000HALF_POS,
					     PHY_MII_CTRL1000_1000HALF_LEN, 0);
	}
	...
	if (phy_ag_adv.full_100m) {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_ADVERTISE_100FULL_POS,
					     PHY_MII_ADVERTISE_100FULL_LEN, 1);
	} else {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_ADVERTISE_100FULL_POS,
					     PHY_MII_ADVERTISE_100FULL_LEN, 0);
	}
	if (phy_ag_adv.half_100m) {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_ADVERTISE_100HALF_POS,
					     PHY_MII_ADVERTISE_100HALF_LEN, 1);
	} else {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_ADVERTISE_100HALF_POS,
					     PHY_MII_ADVERTISE_100HALF_LEN, 0);
	}
	if (phy_ag_adv.full_10m) {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_ADVERTISE_10FULL_POS,
					     PHY_MII_ADVERTISE_10FULL_LEN, 1);
	} else {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_ADVERTISE_10FULL_POS,
					     PHY_MII_ADVERTISE_10FULL_LEN, 0);
	}
	if (phy_ag_adv.half_10m) {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_ADVERTISE_10HALF_POS,
					     PHY_MII_ADVERTISE_10HALF_LEN, 1);
	} else {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PHY_MII_ADVERTISE_10HALF_POS,
					     PHY_MII_ADVERTISE_10HALF_LEN, 0);
	}
...
static void fxgmac_phy_green_ethernet(struct fxgmac_pdata *pdata)
{
	...
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
			      REG_MII_EXT_REG_PMA_DBG0_ADC);
	fxgmac_write_ephy_reg(
		pdata, REG_MII_EXT_DATA,
		REG_MII_EXT_ENABLE_GIGA_POWER_SAVING_FOR_SHORT_CABLE);

	/* CLD */
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
			      REG_MII_EXT_REG_CLD_REG0);
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
			      REG_MII_EXT_ENABLE_CLD_NP_WP);
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
			      REG_MII_EXT_REG_CLD_REG1);
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
			      REG_MII_EXT_ENABLE_CLD_GT_HT_BT);
...
static void fxgmac_phy_eee_feature(struct fxgmac_pdata *pdata)
{
	...
	regval = FXGMAC_SET_REG_BITS(regval, DMA_SBMR_EN_LPI_POS,
				     DMA_SBMR_EN_LPI_LEN, 1);
	regval = FXGMAC_SET_REG_BITS(regval, DMA_SBMR_LPI_XIT_PKT_POS,
				     DMA_SBMR_LPI_XIT_PKT_LEN, 1);
	regval = FXGMAC_SET_REG_BITS(regval, DMA_SBMR_AALE_POS,
				     DMA_SBMR_AALE_LEN, 1);
	...
	regval = FXGMAC_SET_REG_BITS(regval, MAC_LPIET_POS, MAC_LPIET_LEN,
				     MAC_LPI_ENTRY_TIMER);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_LPI_TIMER);

	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_LPI_CONTROL);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_TWT_POS, MAC_TWT_LEN,
				     MAC_TWT_TIMER);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_LST_POS, MAC_LST_LEN,
				     MAC_LST_TIMER);
	...
	regval = FXGMAC_SET_REG_BITS(regval, MAC_MS_TIC_POS, MAC_MS_TIC_LEN,
				     MAC_MS_TIC);
	writereg(pdata->pAdapter, regval, pdata->mac_regs + MAC_MS_TIC_COUNTER);

	fxgmac_write_ephy_mmd_reg(pdata, REG_MMD_EEE_ABILITY_REG, 0x07,
				  REG_MMD_EEE_ABILITY_VALUE);
...
static void fxgmac_reset_phy(struct fxgmac_pdata *pdata)
{
	...
	value = FXGMAC_SET_REG_BITS(value, MGMT_EPHY_CTRL_RESET_POS,
				    MGMT_EPHY_CTRL_RESET_LEN,
				    MGMT_EPHY_CTRL_STA_EPHY_RESET);
...
void fxgmac_release_phy(struct fxgmac_pdata *pdata)
{
	...
	value = FXGMAC_SET_REG_BITS(value, MGMT_EPHY_CTRL_RESET_POS,
				    MGMT_EPHY_CTRL_RESET_LEN,
				    MGMT_EPHY_CTRL_STA_EPHY_RELEASE);
	writereg(pdata->pAdapter, value, pdata->base_mem + MGMT_EPHY_CTRL);
	usleep_range_ex(pdata->pAdapter, 100, 150);
	value = readreg(pdata->pAdapter, pdata->base_mem + MGMT_EPHY_CTRL);
	DBGPRINT(MP_LOUD, ("0x1004: 0x%x\n", value));
#ifdef AISC_MODE
	fxgmac_read_ephy_reg(pdata, REG_MII_SPEC_CTRL,
			     &value); /* read  phy specific control */
	value = FXGMAC_SET_REG_BITS(value, PHY_MII_SPEC_CTRL_CRS_ON_POS,
				    PHY_MII_SPEC_CTRL_CRS_ON_LEN,
				    1); /* set on crs on */
	fxgmac_write_ephy_reg(pdata, REG_MII_SPEC_CTRL,
			      value); /* phy specific control set on crs on */
	...
	value = FXGMAC_SET_REG_BITS(value,
				    MII_EXT_ANALOG_CFG3_ADC_START_CFG_POS,
				    MII_EXT_ANALOG_CFG3_ADC_START_CFG_LEN,
				    MII_EXT_ANALOG_CFG3_ADC_START_CFG_DEFAULT);
	...
	if (EFUSE_LED_COMMON_SOLUTION != value) {
		...
		case EFUSE_LED_SOLUTION1:
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
				REG_MII_EXT_COMMON_LED0_CFG_VALUE_SOLUTION1);
			break;
		case EFUSE_LED_SOLUTION2:
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
				REG_MII_EXT_COMMON_LED0_CFG_VALUE_SOLUTION2);
			break;
		case EFUSE_LED_SOLUTION3:
		case EFUSE_LED_SOLUTION4:
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
				REG_MII_EXT_COMMON_LED0_CFG_VALUE_SOLUTION3);
			break;
		default:
			/* default solution */
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
				REG_MII_EXT_COMMON_LED0_CFG_VALUE_SOLUTION0);
			break;
		}
		...
		case EFUSE_LED_SOLUTION4:
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
				REG_MII_EXT_COMMON_LED1_CFG_VALUE_SOLUTION1);
			break;
		case EFUSE_LED_SOLUTION2:
		case EFUSE_LED_SOLUTION3:
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
				REG_MII_EXT_COMMON_LED1_CFG_VALUE_SOLUTION2);
			break;
		default:
			/* default solution */
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
				REG_MII_EXT_COMMON_LED1_CFG_VALUE_SOLUTION0);
			break;
		}
		...
		case EFUSE_LED_SOLUTION1:
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
				REG_MII_EXT_COMMON_LED2_CFG_VALUE_SOLUTION0);
			break;
		case EFUSE_LED_SOLUTION2:
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
				REG_MII_EXT_COMMON_LED2_CFG_VALUE_SOLUTION2);
			break;
		case EFUSE_LED_SOLUTION3:
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
				REG_MII_EXT_COMMON_LED2_CFG_VALUE_SOLUTION3);
			break;
		case EFUSE_LED_SOLUTION4:
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
				REG_MII_EXT_COMMON_LED2_CFG_VALUE_SOLUTION4);
			break;
		default:
			/* default solution */
			fxgmac_write_ephy_reg(
		...
		if (EFUSE_LED_SOLUTION2 == value) {
			fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
					      REG_MII_EXT_COMMON_LED_BLINK_CFG);
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
...
static void fxgmac_enable_phy_check(struct fxgmac_pdata *pdata)
{
	...
	fxgmac_read_ephy_reg(pdata, REG_MII_EXT_DATA, &value);
	value = FXGMAC_SET_REG_BITS(value, REG_MII_EXT_PKG_CHECK_POS,
				    REG_MII_EXT_PKG_CHECK_LEN,
				    REG_MII_EXT_PKG_ENABLE_CHECK);
...
{
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
			      REG_MII_EXT_SLEEP_CONTROL_REG);
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
			      REG_MII_EXT_SLEEP_REG_ENABLE_LOOPBACK);

	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR, REG_MII_EXT_LPBK_REG);
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
			      REG_MII_EXT_LPBK_REG_ENABLE_LOOPBACK);

	fxgmac_write_ephy_reg(pdata, REG_MII_BMCR,
			      REG_MII_BMCR_ENABLE_LOOPBACK);
}
...
static void fxgmac_clean_cable_loopback(struct fxgmac_pdata *pdata)
{
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
			      REG_MII_EXT_SLEEP_CONTROL_REG);
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
			      REG_MII_EXT_SLEEP_REG_CLEAN_LOOPBACK);

	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR, REG_MII_EXT_LPBK_REG);
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
			      REG_MII_EXT_LPBK_REG_CLEAN_LOOPBACK);

	fxgmac_write_ephy_reg(pdata, REG_MII_BMCR,
			      REG_MII_BMCR_DISABLE_LOOPBACK);
}
...
static void fxgmac_disable_phy_sleep(struct fxgmac_pdata *pdata)
{
	...
	value = FXGMAC_SET_REG_BITS(value, MII_EXT_SLEEP_CONTROL1_EN_POS,
				    MII_EXT_SLEEP_CONTROL1_EN_LEN, 0);
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
			      REG_MII_EXT_SLEEP_CONTROL_REG);
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA, value);
}
...
static void fxgmac_enable_phy_sleep(struct fxgmac_pdata *pdata)
{
	...
	value = FXGMAC_SET_REG_BITS(value, MII_EXT_SLEEP_CONTROL1_EN_POS,
				    MII_EXT_SLEEP_CONTROL1_EN_LEN, 1);
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
			      REG_MII_EXT_SLEEP_CONTROL_REG);
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA, value);
}
...
static void fxmgac_config_led_under_active(struct fxgmac_pdata *pdata)
{
	...
	if (EFUSE_LED_COMMON_SOLUTION == regval) {
		...
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s0_led_setting[0]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED0_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s0_led_setting[1]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED1_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s0_led_setting[2]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED2_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s0_led_setting[3]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED_BLINK_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s0_led_setting[4]);
	}
}
...
static void fxgmac_config_led_under_sleep(struct fxgmac_pdata *pdata)
{
	...
	if (EFUSE_LED_COMMON_SOLUTION == regval) {
		...
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s3_led_setting[0]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED0_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s3_led_setting[1]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED1_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s3_led_setting[2]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED2_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s3_led_setting[3]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED_BLINK_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s3_led_setting[4]);
	}
}
...
static void fxgmac_config_led_under_shutdown(struct fxgmac_pdata *pdata)
{
	...
	if (EFUSE_LED_COMMON_SOLUTION == regval) {
		...
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s5_led_setting[0]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED0_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s5_led_setting[1]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED1_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s5_led_setting[2]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED2_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s5_led_setting[3]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED_BLINK_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.s5_led_setting[4]);
	}
}
...
static void fxgmac_config_led_under_disable(struct fxgmac_pdata *pdata)
{
	...
	if (EFUSE_LED_COMMON_SOLUTION == regval) {
		...
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.disable_led_setting[0]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED0_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.disable_led_setting[1]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED1_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.disable_led_setting[2]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED2_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.disable_led_setting[3]);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
				      REG_MII_EXT_COMMON_LED_BLINK_CFG);
		fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA,
				      pdata->led.disable_led_setting[4]);
	} else {
			     ...
			     enum fxgmac_int int_id)
{
	...
	case FXGMAC_INT_DMA_CH_SR_TI:
		dma_ch_ier = FXGMAC_SET_REG_BITS(dma_ch_ier, DMA_CH_IER_TIE_POS,
						 DMA_CH_IER_TIE_LEN, 1);
		break;
	...
	case FXGMAC_INT_DMA_CH_SR_RI:
		dma_ch_ier = FXGMAC_SET_REG_BITS(dma_ch_ier, DMA_CH_IER_RIE_POS,
						 DMA_CH_IER_RIE_LEN, 1);
		break;
	...
	case FXGMAC_INT_DMA_CH_SR_RPS:
		dma_ch_ier = FXGMAC_SET_REG_BITS(dma_ch_ier, DMA_CH_IER_RSE_POS,
...
static void fxgmac_enable_msix_rxtxinterrupt(struct fxgmac_pdata *pdata)
{
	...
	for (intid = 0; intid < MSIX_TBL_RXTX_NUM; intid++) {
		writereg(pdata->pAdapter, 0,
			 pdata->base_mem + MSIX_TBL_BASE_ADDR +
				 MSIX_TBL_MASK_OFFSET + intid * 16);
	}
}
...
static void fxgmac_enable_msix_rxtxphyinterrupt(struct fxgmac_pdata *pdata)
{
	...
	writereg(pdata->pAdapter, 0,
		 pdata->base_mem + MSIX_TBL_BASE_ADDR + MSIX_TBL_MASK_OFFSET +
			 MSI_ID_PHY_OTHER * 16);
#if !(FUXI_EPHY_INTERRUPT_D0_OFF)
	hw_ops->read_ephy_reg(pdata, REG_MII_INT_STATUS,
			      NULL); /*  clear  phy interrupt */
	regval = FXGMAC_SET_REG_BITS(0, PHY_INT_MASK_LINK_UP_POS,
				     PHY_INT_MASK_LINK_UP_LEN, 1);
	regval = FXGMAC_SET_REG_BITS(regval, PHY_INT_MASK_LINK_DOWN_POS,
				     PHY_INT_MASK_LINK_DOWN_LEN, 1);
	hw_ops->write_ephy_reg(
		pdata, REG_MII_INT_MASK,
		regval); /* enable phy interrupt    ASIC bit10 linkup  bit11 linkdown */
#endif
}
static void fxgmac_enable_msix_one_interrupt(struct fxgmac_pdata *pdata,
					     u32 intid)
{
	writereg(pdata->pAdapter, 0,
		 pdata->base_mem + MSIX_TBL_BASE_ADDR + MSIX_TBL_MASK_OFFSET +
...
static void fxgmac_disable_msix_one_interrupt(struct fxgmac_pdata *pdata,
					      u32 intid)
{
	writereg(pdata->pAdapter, 0x01,
		 pdata->base_mem + MSIX_TBL_BASE_ADDR + MSIX_TBL_MASK_OFFSET +
			 intid * 16);
}
...
static void fxgmac_legacy_link_speed_setting(struct fxgmac_pdata *pdata)
{
	...
	for (i = 0, regval = fxgmac_get_ephy_state(pdata);
	     (!(regval & MGMT_EPHY_CTRL_STA_EPHY_RELEASE) ||
	      !(regval & MGMT_EPHY_CTRL_STA_EPHY_LINKUP)) &&
	     (i < PHY_LINK_TIMEOUT);
	     regval = fxgmac_get_ephy_state(pdata), i++) {
		usleep_range_ex(pdata->pAdapter, 2000, 2000);
	}
	fxgmac_read_ephy_reg(pdata, REG_MII_INT_STATUS,
			     NULL); /* clear  phy interrupt. */
}
				    ...
				    unsigned int wol)
{
	...
	if (EFUSE_LED_COMMON_SOLUTION != regval) {
		...
		if (EFUSE_LED_SOLUTION3 == regval) {
			fxgmac_write_ephy_reg(
				pdata, REG_MII_EXT_DATA,
				REG_MII_EXT_COMMON_LED0_CFG_VALUE_SLEEP_SOLUTION3);
		} else {
			fxgmac_write_ephy_reg(pdata, REG_MII_EXT_DATA, 0x00);
		}
	...
	regval = FXGMAC_SET_REG_BITS(regval, LPW_CTRL_ASPM_LPW_EN_POS,
				     LPW_CTRL_ASPM_LPW_EN_LEN,
				     1); /* Enable PCIE PM_L23. */

	writereg(pdata->pAdapter, regval, pdata->base_mem + LPW_CTRL);

	/* set gmac power down */
	regval = readreg(pdata->pAdapter, pdata->mac_regs + MAC_PMT_STA);
	regval = FXGMAC_SET_REG_BITS(regval, MAC_PMT_STA_PWRDWN_POS,
	 ...
	 * this modification is only temporarif it is 55mv, wol maybe failed.
	 */
...
static void fxgmac_config_powerup(struct fxgmac_pdata *pdata)
{
	...
	regval = FXGMAC_SET_REG_BITS(regval, MGMT_WPI_CTRL0_WPI_MODE_POS,
...
static int fxgmac_diag_sanity_check(struct fxgmac_pdata *pdata)
{
	...
	static u32 reg_tail_pre;
	static int cnt;

	reg_q_val = readreg(pdata->pAdapter,
			    FXGMAC_MTL_REG(pdata, 0 /* tx channe 0 */,
					   0x8 /*  0x2d08 */));
	...
	reg_tail_val =
		readreg(pdata->pAdapter,
			FXGMAC_DMA_REG(pdata->channel_head, DMA_CH_TDTR_LO));
...
static void fxgmac_pwr_clock_gate(struct fxgmac_pdata *pdata)
{
	...
	fxgmac_write_ephy_reg(pdata, REG_MII_EXT_ADDR,
			      REG_MII_EXT_SLEEP_CONTROL1);
	fxgmac_read_ephy_reg(pdata, REG_MII_EXT_DATA, &regval);
	/* close pll in sleep mode */
	regval = FXGMAC_SET_REG_BITS(regval,
				     MII_EXT_SLEEP_CONTROL1_PLLON_IN_SLP_POS,
				     MII_EXT_SLEEP_CONTROL1_PLLON_IN_SLP_LEN,
				     0);
...
static unsigned char fxgmac_suspend_int(void *context)
{
	...
	val_mgmt_intcrtl0 =
		(u32)readreg(pdata->pAdapter, pdata->base_mem + MGMT_INT_CTRL0);
	/* disable  management interrupts. enable only pmt interrupts. */
	val_mgmt_intcrtl0 = FXGMAC_SET_REG_BITS(val_mgmt_intcrtl0,
						MGMT_INT_CTRL0_INT_MASK_POS,
						MGMT_INT_CTRL0_INT_MASK_LEN,
						MGMT_INT_CTRL0_INT_MASK_EX_PMT);
	writereg(pdata->pAdapter, val_mgmt_intcrtl0,
		 pdata->base_mem + MGMT_INT_CTRL0);

	for (intid = 0; intid < MSIX_TBL_MAX_NUM;
	     intid++) { /* disable all msix */
		writereg(pdata->pAdapter, 0x1,
			 pdata->base_mem + MSIX_TBL_BASE_ADDR +
				 MSIX_TBL_MASK_OFFSET + intid * 16);
	}
	...
	/* enable pmt msix */
	writereg(pdata->pAdapter, 0x0,
		 pdata->base_mem + MSIX_TBL_BASE_ADDR + MSIX_TBL_MASK_OFFSET +
			 MSI_ID_PHY_OTHER * 16);
	readreg(pdata->pAdapter,
		pdata->base_mem +
			MGMT_WOL_CTRL); /* read clear wake up reason */
	...
	regval = FXGMAC_SET_REG_BITS(regval, PHY_INT_MASK_LINK_DOWN_POS,
				     PHY_INT_MASK_LINK_DOWN_LEN, 1);
	fxgmac_write_ephy_reg(pdata, REG_MII_INT_MASK,
			      regval); /* enable phy interrupt */
#endif
...
static int fxgmac_suspend_txrx(struct fxgmac_pdata *pdata)
{
	...
	do {
		regval =
			readreg(pdata->pAdapter, pdata->mac_regs + MAC_DBG_STA);
		busy--;
	...
	for (i = 0; i < pdata->tx_q_count; i++) {
		do {
			regval = readreg(pdata->pAdapter,
					 FXGMAC_MTL_REG(pdata, i, MTL_TXQ_DEG));
			busy--;
		} while ((regval & MTL_TXQ_DEG_TX_BUSY) && (busy));
		if (0 != (regval & MTL_TXQ_DEG_TX_BUSY)) {
			regval = readreg(pdata->pAdapter,
					 pdata->mac_regs + MTL_TXQ_DEG);
			DbgPrintF(
				MP_WARN,
				"warning !!!timed out waiting for tx queue %u to empty\n",
				i);
			return -1;
		}
	}
...
static void fxgmac_resume_int(struct fxgmac_pdata *pdata)
{
	...
	val_mgmt_intcrtl0 =
		(u32)readreg(pdata->pAdapter, pdata->base_mem + MGMT_INT_CTRL0);
	/* disable management interrupts. enable only pmt interrupts. */
	val_mgmt_intcrtl0 = FXGMAC_SET_REG_BITS(
		val_mgmt_intcrtl0, MGMT_INT_CTRL0_INT_MASK_POS,
		MGMT_INT_CTRL0_INT_MASK_LEN, MGMT_INT_CTRL0_INT_MASK_DISABLE);
	writereg(pdata->pAdapter, val_mgmt_intcrtl0,
		 pdata->base_mem + MGMT_INT_CTRL0);
	...
	for (intid = MSIX_TBL_RXTX_NUM; intid < MSIX_TBL_MAX_NUM;
	     intid++) { /* disable some msix */
		writereg(pdata->pAdapter, 0,
			 pdata->base_mem + MSIX_TBL_BASE_ADDR +
				 MSIX_TBL_MASK_OFFSET + intid * 16);
	}
...
static int fxgmac_hw_init(struct fxgmac_pdata *pdata)
{
	...
	if (netif_msg_drv(pdata)) {
		DPRINTK("fxgmac hw init call in\n");
	}
	...
	if (netif_msg_drv(pdata)) {
		DPRINTK("fxgmac hw init callout\n");
	}
	return 0;
}
...
static void fxgmac_esd_restore_pcie_cfg(struct fxgmac_pdata *pdata)
{
	...
	cfg_w32(pdata, REG_CACHE_LINE_SIZE,
		pdata->expansion.cfg_cache_line_size);
...
static int fxgmac_hw_exit(struct fxgmac_pdata *pdata)
{
	...
	pdata->pcie_link_status =
		FXGMAC_GET_REG_BITS(regval, PCI_LINK_CTRL_ASPM_CONTROL_POS,
				    PCI_LINK_CTRL_ASPM_CONTROL_LEN);
	if (PCI_LINK_CTRL_L1_STATUS == (pdata->pcie_link_status & 0x02)) {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PCI_LINK_CTRL_ASPM_CONTROL_POS,
					     PCI_LINK_CTRL_ASPM_CONTROL_LEN, 0);
	...
	fxgmac_restore_nonstick_reg(
		pdata); /* reset will clear nonstick registers. */
...
static int fxgmac_pcie_init(struct fxgmac_pdata *pdata, bool ltr_en,
			    ...
			    bool aspm_l0s_en)
{
	...
	cfg_r32(pdata, REG_PCI_LINK_CTRL, &regval);
	if (PCI_LINK_CTRL_L1_STATUS == (pdata->pcie_link_status & 0x02) &&
	    0x00 == FXGMAC_GET_REG_BITS(regval, PCI_LINK_CTRL_ASPM_CONTROL_POS,
					PCI_LINK_CTRL_ASPM_CONTROL_LEN)) {
		regval = FXGMAC_SET_REG_BITS(regval,
					     PCI_LINK_CTRL_ASPM_CONTROL_POS,
					     PCI_LINK_CTRL_ASPM_CONTROL_LEN,
	...
	regval = FXGMAC_SET_REG_BITS(0, LTR_IDLE_ENTER_REQUIRE_POS,
				     LTR_IDLE_ENTER_REQUIRE_LEN,
				     LTR_IDLE_ENTER_REQUIRE);
	regval = FXGMAC_SET_REG_BITS(regval, LTR_IDLE_ENTER_SCALE_POS,
				     LTR_IDLE_ENTER_SCALE_LEN,
				     LTR_IDLE_ENTER_SCALE);
	regval = FXGMAC_SET_REG_BITS(regval, LTR_IDLE_ENTER_POS,
				     LTR_IDLE_ENTER_LEN, LTR_IDLE_ENTER_USVAL);
	regval = (regval << 16) + regval; /* snoopy + non-snoopy */
	writereg(pdata->pAdapter, regval, pdata->base_mem + LTR_IDLE_ENTER);

	regval = 0;
	regval = FXGMAC_SET_REG_BITS(0, LTR_IDLE_EXIT_REQUIRE_POS,
				     LTR_IDLE_EXIT_REQUIRE_LEN,
				     LTR_IDLE_EXIT_REQUIRE);
	regval = FXGMAC_SET_REG_BITS(regval, LTR_IDLE_EXIT_SCALE_POS,
				     LTR_IDLE_EXIT_SCALE_LEN,
				     LTR_IDLE_EXIT_SCALE);
	regval = FXGMAC_SET_REG_BITS(regval, LTR_IDLE_EXIT_POS,
				     LTR_IDLE_EXIT_LEN, LTR_IDLE_EXIT_USVAL);
	regval = (regval << 16) + regval; /* snoopy + non-snoopy */
	writereg(pdata->pAdapter, regval, pdata->base_mem + LTR_IDLE_EXIT);

	regval = readreg(pdata->pAdapter, pdata->base_mem + LTR_CTRL);
	if (ltr_en) {
		regval = FXGMAC_SET_REG_BITS(regval, LTR_CTRL_EN_POS,
					     LTR_CTRL_EN_LEN, 1);
		regval = FXGMAC_SET_REG_BITS(regval,
					     LTR_CTRL_IDLE_THRE_TIMER_POS,
	...
	regval = readreg(pdata->pAdapter, pdata->base_mem + LPW_CTRL);
	regval = FXGMAC_SET_REG_BITS(regval, LPW_CTRL_ASPM_L0S_EN_POS,
				     LPW_CTRL_ASPM_L0S_EN_LEN,
				     aspm_l0s_en ? 1 : 0);
	regval = FXGMAC_SET_REG_BITS(regval, LPW_CTRL_ASPM_L1_EN_POS,
				     LPW_CTRL_ASPM_L1_EN_LEN,
				     aspm_l1_en ? 1 : 0);
	regval = FXGMAC_SET_REG_BITS(regval, LPW_CTRL_L1SS_EN_POS,
				     LPW_CTRL_L1SS_EN_LEN,
				     aspm_l1ss_en ? 1 : 0);
	writereg(pdata->pAdapter, regval, pdata->base_mem + LPW_CTRL);

	cfg_r32(pdata, REG_ASPM_CONTROL, &regval);
	regval = FXGMAC_SET_REG_BITS(regval, ASPM_L1_IDLE_THRESHOLD_POS,
				     ASPM_L1_IDLE_THRESHOLD_LEN,
				     ASPM_L1_IDLE_THRESHOLD_1US);
	cfg_w32(pdata, REG_ASPM_CONTROL, regval);

	regval = 0;
	regval = FXGMAC_SET_REG_BITS(regval, PCIE_SERDES_PLL_AUTOOFF_POS,
				     PCIE_SERDES_PLL_AUTOOFF_LEN, 1);
	writereg(pdata->pAdapter, regval,
		 pdata->base_mem + REG_PCIE_SERDES_PLL);
	...
	if (FUXI_REV_01 == regval && PCI_DEVICE_ID_FUXI == deviceid) {
		regval =
			readreg(pdata->pAdapter, pdata->base_mem + MGMT_SIGDET);
		regval = FXGMAC_SET_REG_BITS(regval, MGMT_SIGDET_POS,
					     MGMT_SIGDET_LEN, MGMT_SIGDET_55MV);
		writereg(pdata->pAdapter, regval,
			 pdata->base_mem + MGMT_SIGDET);
	}
...
void fxgmac_init_hw_ops(struct fxgmac_hw_ops *hw_ops)
{
	...
	hw_ops->enable_msix_rxtxphyinterrupt =
		fxgmac_enable_msix_rxtxphyinterrupt;
	...
	hw_ops->check_wake_pattern_fifo_pointer =
		fxgmac_check_wake_pattern_fifo_pointer;
	...
	hw_ops->config_multicast_mac_hash_table =
	...
	hw_ops->read_patch_from_efuse =
		fxgmac_read_patch_from_efuse; /* read patch per register. */
	hw_ops->read_patch_from_efuse_per_index =
		fxgmac_read_patch_from_efuse_per_index; /* read patch per index. */
	hw_ops->write_patch_to_efuse = fxgmac_write_patch_to_efuse;
	hw_ops->write_patch_to_efuse_per_index =
		fxgmac_write_patch_to_efuse_per_index;
